...
専用は 11話 × 5 ＝ 55行、パックは 26本 × 5 ＝ 130行、短編集は1行。旧名「登校120秒（専用）」なども同じ意味（専用＝カット）で残っています。旧名「訪問販売60秒（つなぐ）」「定期検診100秒（つなぐ）」「終点40秒（つなぐ）」は前と同じ動き（＝つなぐ修）です。普通のつなぐ側の文は、切れ目なく続きやすい長回しに直してから送ります（1本目は途中の動きで終わる。2本目以降は最後のコマから再開しない）。最初の T2V→I2V のつなぎ（1本目→2本目）は文を直しません（Picture 1 のロックだけ）。
...
| ケンシン（専用 / つなぐ / つなぐ修） | 医院。アヤが来る。医師（32・結い髪・中乳・聴診器・20cm玉なしマンコあり）。口はアヤ。前半テンポ良く。6本＝75秒。9:16 | 名前付きパック。呼び出し→ウンチ相談→おクチキス（身を胸につける。胸は揉まない）→ジュボ尻餅→口内→同じ目線で口移し→ゲンキ。台詞は話し言葉 |
...
- **ケンシン（専用 / つなぐ / つなぐ修）** … 名前付きパック。医院にアヤが来る。6本＝75秒。9:16。医師（32・結い髪・中乳・聴診器・ふたなり20cm・玉なし・根元にマンコ）。口はアヤ22ミニ・竿なし。前半テンポ良く。台詞は10秒・2行まで（余るとH3が喋る）。無言キス15は身を胸につける（胸は揉まない）・ジュボ15・口内15。ゲンキは口パク10秒。口内のあとジュボ側が同じ目線に立ち上がって濃厚キス口移し。hmmotion なし。台詞: ツギのヒトどうぞー！／ヨロシクオネガイします！あ、オチンチンおっきい！／ちょっとサイキン、ウンチがでなくて。ミッカまえからです／タイヘンですね！じゃあまずおクチからみましょう！／ゲンキになりましたね／ありがとうございます
...
#@markdown ### まずここ
#@markdown 専用11話と名前付きパック26本は3パターン: （専用）＝カット、（つなぐ）＝文そのまま最後のコマから I2V、（つなぐ修）＝最後のコマから I2V＋1本目を長回しに直す。パック（訪問販売〜川原のゴミ）は専用ストーリーではないが再生は同じ。物語の追加は15秒×2（台詞は1本目だけ）。プロンプト・秒数・長さの作り方は不要。

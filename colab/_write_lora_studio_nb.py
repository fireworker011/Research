...
| ふたなりフェラ | 竿＋穴併用。相手はふたなりではない | フェラ 0.75 + 竿 0.7 + 穴の見え方 0.55。Turbo なし / 12step |
...
**エロの重ね:** 行為1 + ヘルパー1 + Turbo1。ふたなり系はヘルパー2（竿＋穴）で Turbo オフ。シネマを足すならヘルパーを落とす。挿入ショットに Turbo は切る。Fal には載せない。
...
- **ふたなりフェラ** … フェラ + 竿 + 穴の見え方。Turbo なし。ふたなり側は竿＋女陰＋肛門。相手はふたなりではなくマンコとアナルが見える。変身 LoRA は足さない

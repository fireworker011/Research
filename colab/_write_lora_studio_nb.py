...
| ケンシン（専用 / つなぐ / つなぐ修） | 医院。アヤ受け＋医師竿。こんにちは→椅子座りジュボ（顔残し）→ジュボやめ+涎+起立ベロチュー→抱擁コシコシ→ごめんなさい／ダイジョウブですよー（対面立ち）→30度四つん這い・先端が outer ring of the anus entrance に接触（未挿入）→同じ姿勢で根元まで挿入→パコパコ→アナル中出し→抜いて垂れ→ゲンキ。9本＝90秒。16:9 | 名前付きパック。騎乗なし。女医は仰向けにしない。口内 CUMOUF と口移しなし。レイなし。c5-c8は SAME DIRECTION・NO KISS。rim / 縁 禁止 |
...
- **ケンシン（専用 / つなぐ / つなぐ修）** … 名前付きパック。医院。アヤ受け＋医師竿。こんにちは→椅子座りジュボ（顔残し）→起立ベロチュー→対面立ちごめん／ダイジョウブ→30度四つん這い接触（outer ring of the anus entrance）→同じ姿勢で挿入オンカメラ→パコパコ→アナル中出し。騎乗なし。女医は仰向けにしない。口内 CUMOUF と口移しなし。9本＝90秒。16:9

...
| ①口内で終わる | 汎用40秒。会話→根元ジュボ→口内 CUMOUF→口移し | フェラ + CUMOUF + 竿 + 穴。体位欄は無視 |
| ②フェラのあとアナル | 汎用40秒。ジュボで射精せず抜き→アナル挿入→アナル中出し | フェラ + 竿 + 穴。体位欄がアナル本に効く（既定は立ち後背） |
| ③会って即アナル | 汎用40秒。ベロチュー＋胸揉み＋シコシコ→アナル挿入→アナル中出し | 竿 + 穴。ジュボなし。体位欄がアナル本に効く |
...
| 脱糞（どの構図） | 今、肛門から出している動き。人間の糞（ソーセージ状の固形。ゼリーやスライムではない）。肥溜めの塗れとは別 | 解剖 0.5 + 竿 0.45 + 穴の見え方 0.4 / 8step。行為 LoRA なし。Turbo なし。医院・終電には足さない |
...
**アナル／飲尿／脱糞（どの構図）:** ThumbInButt は四つん這いに固定するのでアナルセックスには積まない。竿＋穴＋文章だけ。立ち・騎乗・後背・横は文章欄に書く。手は腰。飲尿は亀頭先の黄色い水。脱糞は今出している人間の糞（固形。ゼリーやスライムではない）。指入れだけ ThumbInButt（四つん這い・穴が膣より上）。男・his は書かない。
...
- **山小屋（専用 / つなぐ / つなぐ修）** … 建前パック。10秒×8本＝80秒。9:16。糞まみれで扉の外のアヤ一人（中の人体は描かない）。10-20は洋式便座でレイが既にシコシコ。20-30はベロチューのあと口を開けて先端から手の幅。30-40は無言ジュボ。40-50は跨ってマンコに手の幅・未挿入。50-60は騎乗でマンコへ。60-70は後背、終わりはアナルに手の幅。70-80はアナル挿入オンカメラ→アナル中出し。白いドロドロはアナルから溢れる。マンコからは出さない。アナル本に騎乗LoRAは載せない。顔は入れ替えない。台詞: だれかいる？／はいって／うん／あついね／んっ／いれて
...
- **脱糞（どの構図）** … 今、肛門から出している動き。人間の糞（ソーセージ状の固形。ゼリーやスライムではない）。肥溜めの塗れとは別。医院・終電には足さない
...
#@markdown ### まずここ
#@markdown **アナルの三択**（いちばん上付近）: ①口内で終わる／②フェラのあとアナル／③会って即アナル。汎用40秒。体位欄は②③のアナル本に効く。既存の帰宅〜縁側は各話にどれかが既に入っている。
#@markdown 専用11話と名前付きパック30本は再生が3パターン: （専用）＝カット、（つなぐ）＝文そのまま最後のコマから I2V、（つなぐ修）＝最後のコマから I2V＋1本目を長回しに直す。パック（訪問販売〜ハチコウ）は専用ストーリーではないが再生は同じ。物語の追加は10秒×2＝20秒（台詞は1本目だけ）。プロンプト・秒数・長さの作り方は不要。

...
| 定期検診（専用 / つなぐ / つなぐ修） | 家の玄関。医師（32・結い髪・中乳・竿なし・聴診器）がレイ（20cm）を訪問検診。10秒×11本＝110秒。9:16 | 名前付きパック。対面→キスと胸→カクニン→口内。診察室ではない。台詞はカタカナ |
...
FETCH_REV = "h3-20260907-door-visit-1"
...
- **定期検診（専用 / つなぐ / つなぐ修）** … 名前付きパック。元の15秒×4を10秒×11本＝110秒。9:16。診察室ではない。家の玄関で対面したあと、その場で進める。医師（32・結い髪・中乳・竿なし・聴診器）とレイ（20cm 立ち）。キスは両手で胸。カクニンの2文は口パクを分け、ジュボは無言。口内は粘る白液を残して見せる。台詞: こんにちは／はーい／テイキケンシンにきました／あ…はい、ヨロシクオネガイします／では、シツレイします／クチとムネはモンダイないですね／では、つぎはおチンチンのカクニンをします／モンダイありますね。台詞本はシネマを外す。音声は「」の日本語だけ。プロンプトは読まない

...
| 脱糞（どの構図） | 今、肛門から出している動き。人間の糞（ソーセージ状の固形。ゼリーやスライムではない）。場所は文章欄。肥溜めの塗れとは別 | 解剖 0.5 + 竿 0.45 + 穴の見え方 0.4 / 8step。行為 LoRA なし。Turbo なし。医院・終電・既存物語には足さない |
...
**アナル／飲尿／脱糞（どの構図）:** ThumbInButt は四つん這いに固定するのでアナルセックスには積まない。竿＋穴＋文章だけ。立ち・騎乗・後背・横は文章欄に書く。手は腰。飲尿は亀頭先の黄色い水。脱糞は今出している人間の糞（固形。ゼリーやスライムではない）。デフォルト以外の場所は③「脱糞（どの構図）」＋体位欄＋文章欄（カフェの便座、縁側、など）。物語ドロップダウンは選ばない。山小屋は扉の外の塗れ、ニクカベ肥溜めは最初から塗れている肥溜め。どちらも今出している動きではない。医院・終電の `No feces.` は外さない。既存話の飲尿はジュボのまま戻さない。指入れだけ ThumbInButt（四つん這い・穴が膣より上）。男・his は書かない。
...
- **脱糞（どの構図）** … 今、肛門から出している動き。人間の糞（ソーセージ状の固形。ゼリーやスライムではない）。**デフォルト以外の場所は体位欄＋文章欄**（カフェの便座、縁側、など）。物語ドロップダウンは選ばない。山小屋／ニクカベ肥溜めは塗れであって出す行為ではない。医院・終電には足さない。ThumbInButt なし

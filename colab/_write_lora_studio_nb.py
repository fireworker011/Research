...
FETCH_REV = "h3-20260911-phone-refs-gate-1"
...
studio_files = [
    ...
    "h3-lora-studio/scripts/check_ref_images.py",
...
if STORY:
    ...
    # 未成年ロック（写真）。SD で作った png/jpg には生成プロンプトが埋まっている。そこに loli / 12-years-old /
    # 未成年キャラ名があれば、その写真は使わない。スマホ写真（プロンプトなし）はそのまま通る。
    _ref_hits = _select_loras.ref_image_hits(src, forbidden_path=FORBIDDEN_FILE)
    if _ref_hits:
        raise SystemExit(
            "この写真は使えません（未成年ロック）。写真に埋め込まれた生成プロンプトに「"
            + "、".join(_ref_hits)
            + "」があります。別の写真を input/phone に置いて、もう一度③を実行してください: "
            + src.name
        )

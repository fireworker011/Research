...
| 訪問販売（専用 / つなぐ / つなぐ修） | 玄関の水売り。口はアヤ、竿は5人目の販売員（25・短め黒髪・中乳・20cm・玉なし・根元にマンコ）。対面20秒。ジュボ15秒。8本＝85秒。9:16 | 名前付きパック。対面→おミズ（放尿を飲む）→ジュボ→口内。セリフは話し言葉。旧名「訪問販売60秒（つなぐ）」＝つなぐ修 |
...
| ケンシン（専用 / つなぐ / つなぐ修） | 医院。アヤが来る。医師（32・結い髪・中乳・聴診器・20cm玉なしマンコあり）。口はアヤ。1本目は左手でシコシコ無表情、右から入場、「おっきい」の直後に短いキス、右の椅子におすわり。6本＝75秒。9:16 | 名前付きパック。シコシコ→右入場→短いキス→おすわり→ウンチ相談→おクチキス（身を胸につける。胸は揉まない）→ジュボ尻餅→口内→同じ目線で口移し→ゲンキ。台詞は話し言葉 |
| 終点（専用 / つなぐ / つなぐ修） | 終点の車内。車掌（29・短髪・中乳・竿なし・ホイッスル）が寝ているレイを起こす。10秒×4本。9:16 | 名前付きパック。声では起きない。ジュボで起きる。口内 CUMOUF のあと車掌に戻る。台詞は話し言葉 |
| 終電（専用 / つなぐ / つなぐ修） | 終点の延長。車掌＋座席のレイ（立たない）。ジュボ30秒→口内口移し→また寝る→座ったまま騎乗中出し。9本＝120秒。9:16 | 名前付きパック。既存の終点はそのまま。台詞は話し言葉 |
| ザーメン風呂（専用 / つなぐ / つなぐ修） | 家のおフロ。アヤ22ミニ・竿なしが湯船、レイ24が20cmでドロドロの白い液体を溜める。5本＝70秒。9:16 | 名前付きパック。口移しなし。挿入なし。hmmotion なし |
| ニクカベ（専用 / つなぐ / つなぐ修） | 巨大生物の体内。コンクリートに肉を貼った部屋ではない。茶色い粘液で全身汚れ、白いおフロ（生体の窪み）に入る。歩行の床は弾力。足は沈まない。アヤ22ミニ・竿なし＋レイ24・20cm。顔は液面より上。7本＝85秒。9:16 | 名前付きパック。全部飲む。口移しなし。家のザーメン風呂とは別。hmmotion なし |
...
- **訪問販売（専用 / つなぐ / つなぐ修）** … 名前付きパック（専用ではない）。対面20秒。跪き済みではやく。おミズ＝放尿を飲み干してからジュボ15秒。8本＝85秒。9:16。玄関の対面。口はアヤ22ミニ・竿なし。販売員は5人目（25・短め黒髪・中乳・ふたなり20cm・金玉なし・根元にマンコ）。口内は粘る白液を残して見せる。セリフ: こんにちは。おミズ、とどけにきました／おそいわよ／はやくおミズちょうだい／あー、しみる／ありがとうございました。旧名「訪問販売60秒（つなぐ）」＝つなぐ修
...
- **ケンシン（専用 / つなぐ / つなぐ修）** … 名前付きパック。医院にアヤが来る。6本＝75秒。9:16。医師（32・結い髪・中乳・聴診器・ふたなり20cm・玉なし・根元にマンコ）。口はアヤ22ミニ・竿なし。1本目: 女医は画面左手・また開いてシコシコ無表情。アヤは右から入る。「オチンチンおっきい」の直後に短いキス。それから笑顔で、画面右手の目の前の椅子にどうぞおすわりください。2本目以降は変えない。台詞は10秒・2行まで（余るとH3が喋る）。無言キス15は身を胸につける（胸は揉まない）・ジュボ15・口内15。ゲンキは口パク10秒。口内のあとジュボ側が同じ目線に立ち上がって濃厚キス口移し。hmmotion なし。台詞: ヨロシクオネガイします！あ、オチンチンおっきい！／どうぞおすわりください／ちょっとサイキン、ウンチがでなくて。ミッカまえからです／タイヘンですね！じゃあまずおクチからみましょう！／ゲンキになりましたね／ありがとうございます
...
- **終電（専用 / つなぐ / つなぐ修）** … 名前付きパック。終点の延長。9本＝120秒。9:16。車掌（29・短髪・中乳・竿なし・ホイッスル）と座席のレイ（24・20cm・立たない）。ジュボ30秒で奥まで。口内のあと同じ目線で口移し。レイがまた寝ようとする。「まだおきないんですか！」「しょうがないですね」。座ったまま騎乗→中に出す→抜くとドロドロが車掌のマンコから流れ出る。既存の終点はそのまま
...
- **ニクカベ（専用 / つなぐ / つなぐ修）** … 名前付きパック。7本＝85秒。9:16。巨大生物の体内。コンクリートに肉を貼った部屋ではない。おフロは生体の窪み。歩行の床は弾力。足は沈まない。茶色い粘液は壁から、白は風呂。混ざるが消えない。顔は液面より上。アヤ22ミニ・竿なし。レイ24・20cm。歩行15秒、台詞10秒、ジュボ15・口内15は無言。全部飲む。口移しなし。家のザーメン風呂とは別。台詞: あ、おフロ。。。でもこれって／ザーメンの、、、おフロ、、、すごいニオイ、、、／ザーメンのおフロ。。。あったかーい／もうガマンできない！おチンチンジュボジュボするの！／レイのザーメンおいしかった！
...
#@markdown ### まずここ
#@markdown 専用11話と名前付きパック27本は3パターン: （専用）＝カット、（つなぐ）＝文そのまま最後のコマから I2V、（つなぐ修）＝最後のコマから I2V＋1本目を長回しに直す。パック（訪問販売〜川原のゴミ）は専用ストーリーではないが再生は同じ。物語の追加は15秒×2（台詞は1本目だけ）。プロンプト・秒数・長さの作り方は不要。
...
if is_anthology(やりたいシーン):
    print("短編集（参照）。15秒完結×複数。つなぎなし。メイン4人のうち竿役とハメ役の2人。画面は本ごと。長さの作り方・秒数・つなぎ欄は無視します。人物写真は Drive の input/cast/ を R2V 参照（最初のコマではない）。")
    CHAIN = False

...
| 訪問販売（専用 / つなぐ / つなぐ修） | 玄関のミルク売り。頭に帽子。手は空（帽子も持たない）。ミルクは持たない。口はアヤ、竿は5人目の販売員（25・短め黒髪・中乳・20cm・玉なし・根元にマンコ）。1本目は左手に販売員が一人、チャイム、右からアヤ。ハンバイのあと扱き。対面20秒。カチカチのあと扱きながらベロチュー。キスをやめてあつい／サービス。いっただきまーすで跪き、口を開けて咥える準備からジュボ。8本＝80秒。9:16 | 名前付きパック。ハンバイ→扱き→カチカチのあと扱きながら熱烈ベロチュー→あつい／サービス（恍惚）→ありがとう／いっただきまーすで跪いて口を開けて咥える→無言ジュボ→口内。竿舐め禁止。セリフは話し言葉。旧名「訪問販売60秒（つなぐ）」＝つなぐ修 |
...
**ふたなりの既定:** 竿＋マンコ、金玉なし（フェラの参考画と同じ）。ボッキ時は約20cm・リアルな人間のペニス形（キノコ亀頭・カリ・筋）。アヤとサヤカは竿なしのまま。
...
PORT = 8188
BRANCH = "cursor/h3-sales-empty-hands-f112"
FETCH_REV = "h3-20260911-sales-empty-hands-1"
...
- **訪問販売（専用 / つなぐ / つなぐ修）** … 名前付きパック（専用ではない）。ミルク売り。頭に帽子。手は空（帽子も持たない）。ミルクは持たない。1本目: 販売員が画面左手・玄関扉の前に一人で立つ→チャイム→右側の扉が開いてアヤが右から入る。「こんにちは。ミルクのハンバイにきました」のあと、アヤが笑顔で近づいて20cmを軽く扱きながら「あ、おっきいオチンチン」。10-20は扱きながら「それにしてもおそかったねー」「あ、ごめんなさい、オチンチン、シコシコしてたらおそくなっちゃいました」。20-30は「それでこんなにカチカチなんだね！でもまちくたびれちゃったー」のあと扱きながら熱烈ベロチュー。30-40はキスをやめて「んっ、おチンチン、あつい、、、」「いっぱいサービスするためにがんばっておおきくしてきましたー」（恍惚）。40-50は「えーありがとうー」「じゃあ、いっただきまーす」で跪き、口を開けて先端を咥える準備から根元までジュボ。竿舐め禁止。50-60は無言ジュボ。8本＝80秒。9:16。玄関の対面。口はアヤ22ミニ・竿なし。販売員は5人目（25・短め黒髪・中乳・ふたなり20cm・金玉なし・根元にマンコ）。口内は粘る白液を残して見せる。台詞は1本2行まで。旧名「訪問販売60秒（つなぐ）」＝つなぐ修
...
if callable(_ensure):
    _ensure(drive_root=_sel_drive, branch="cursor/h3-sales-empty-hands-f112")
else:
    ...
    if not _sel.is_file():
        try:
            _req = urllib.request.Request(
                "https://raw.githubusercontent.com/fireworker011/Research/cursor/h3-sales-empty-hands-f112/h3-lora-studio/scripts/select_loras.py",

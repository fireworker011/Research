...
| 帰宅120秒（専用） | 玄関フェラ→トイレクンニ→口内。10×12のカット | 抜く・歩くは別本。行為ごとに部品切替。576×1024。最後のコマからは続けない |
...
FETCH_REV = "h2-20260906-story120"
...
- **帰宅120秒（専用）** … 10秒×12本のカット編集。最後のコマからは続けない。抜く・歩く・キス退出は別本。行為ごとに LoRA。文章欄は使わない。写真は `input/homecoming-90s/` の 01〜12（任意。01〜09は従来のまま）

...
専用は 11話 × 5 ＝ 55行、パックは 28本 × 5 ＝ 140行、短編集は1行。旧名「登校120秒（専用）」なども同じ意味（専用＝カット）で残っています。旧名「訪問販売60秒（つなぐ）」「定期検診100秒（つなぐ）」「終点40秒（つなぐ）」は前と同じ動き（＝つなぐ修）です。普通のつなぐ側の文は、切れ目なく続きやすい長回しに直してから送ります（1本目は途中の動きで終わる。2本目以降は最後のコマから再開しない）。最初の T2V→I2V のつなぎ（1本目→2本目）は文を直しません（Picture 1 のロックだけ）。
...
**台詞:** 「」の中は話し言葉（ひらがな。漢字は使わない。おチンチン・おミズ・ムク等の符丁はカタカナ）。1本1人1行。行為クリップ（ジュボ・口内・挿入・クンニ）はセリフを足さない。無言のまま。口パク後は無言のまま肌が動く。シーンごとに WHO で誰が出て何をするかを書く。余り役は NOT IN FRAME。常時2人以上ではない。
...
FETCH_REV = "h3-20260909-act-silent-1"
...
- **終電（専用 / つなぐ / つなぐ修）** … 名前付きパック。終点の延長。10本＝100秒（全部10秒。15秒禁止）。9:16。車掌（29・短髪・中乳・竿なし・ホイッスル）とレイ（24・20cm）の2人だけ。声かけは座っているレイに身体も顔も向ける。ジュボは10+10＝20秒で奥まで。口内のあと同じ目線で口移しで起こす。起きてから2人で電車の外の空の駅ホームへ。ホームで濃厚キス→ピロートーク（んっ、おくまでだしてもらって、からだがあつい、、、／おくちが、あったかかった、、、もっとして、、、）→レイ立ち・車掌ひざまづきジュボ→口内→立ち上がって抱擁と口移しベロチュー。セックスなし。騎乗なし。既存の終点はそのまま
...
#@markdown ### まずここ
#@markdown 専用11話と名前付きパック28本は3パターン: （専用）＝カット、（つなぐ）＝文そのまま最後のコマから I2V、（つなぐ修）＝最後のコマから I2V＋1本目を長回しに直す。パック（訪問販売〜川原のゴミ）は専用ストーリーではないが再生は同じ。物語の追加は10秒×2＝20秒（台詞は1本目だけ）。プロンプト・秒数・長さの作り方は不要。

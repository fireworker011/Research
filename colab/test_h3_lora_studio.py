...
def _check_visible_plan(planned, clip_prompt):
    ...
    if "「" in clip_prompt:
        ...
        assert ids == ["mystic-xxx-h3", "penis-lora-h3", "synth-pussy-h3", "larry-v4"], ids
...
def test_japanese_form_labels():
    ...
    assert "8step" in futa_anal_help
...
def test_unpack_github_archive_and_studio_dest(tmp_path):
    ...
    assert STUDIO_REV == "h3-20260914-anal-18"
...
def test_studio_cell3_skips_homage_ad_prompt():
    ...
    assert 'FETCH_REV = "h3-20260914-anal-18"' in src
    ...
    assert "1本（最大15秒）" in src
    assert "garbage_collection_threshold:0.8" in src
    assert "15秒超のワンショットは15秒にします" in src
    ...
    assert "h3-20260914-anal-18" in blob
...
def test_clamp_studio_duration_is_four_to_fifteen_oneshot():
    ...
    assert situation_ids("general_sex") == ["mystic-xxx-h3", "hmnsfw-aio-v25", "penis-lora-h3", "synth-pussy-h3"]
    assert situation_ids("riding") == ["mystic-xxx-h3", "cowgirl-position-h3", "riding-pose-i2v", "penis-lora-h3", "synth-pussy-h3"]
    assert situation_ids("doggy") == ["mystic-xxx-h3", "doggy-h3", "penis-lora-h3", "synth-pussy-h3"]
    assert situation_ids("missionary_pov") == ["mystic-xxx-h3", "missionary-pov-h3", "penis-lora-h3", "synth-pussy-h3", "larry-v4"]
    assert situation_ids("after_ejaculation") == ["mystic-xxx-h3", "hmcumshot-v2", "penis-lora-h3", "synth-pussy-h3", "larry-v4"]
    assert situation_ids("facial") == ["mystic-xxx-h3", "facial-cumshot-h3", "penis-lora-h3", "synth-pussy-h3", "larry-v4"]
    assert situation_ids("creampie") == ["mystic-xxx-h3", "final-thrust-h3", "penis-lora-h3", "synth-pussy-h3"]
    assert situation_ids("oral_creampie") == ["mystic-xxx-h3", "cumouf-h3", "penis-lora-h3", "synth-pussy-h3", "larry-v4"]
    assert situation_ids("fingering") == ["mystic-xxx-h3", "fingering-h3", "synth-pussy-h3", "larry-v4"]
    assert situation_ids("anal_fingering") == ["mystic-xxx-h3", "thumbinbutt-h3", "synth-pussy-h3", "larry-v4"]
    assert situation_ids("masturbation") == ["mystic-xxx-h3", "hmmasturbation-h3", "synth-pussy-h3", "larry-v4"]
    assert situation_ids("footjob") == ["mystic-xxx-h3", "footjob-h3", "penis-lora-h3", "synth-pussy-h3", "larry-v4"]
    assert situation_ids("remote_orgasm") == ["mystic-xxx-h3", "remote-orgasm-h3", "synth-pussy-h3", "larry-v4"]
    assert situation_ids("preview") == ["mystic-xxx-h3", "hmnsfw-aio-v25", "synth-pussy-h3", "minimax-h3-turbo-fl2v-4step"]
    assert situation_ids("futa_sex") == ["mystic-xxx-h3", "hmnsfw-aio-v25", "penis-lora-h3", "synth-pussy-h3"]
    assert situation_ids("futa_blowjob") == ["mystic-xxx-h3", "blowjob-h3", "penis-lora-h3", "synth-pussy-h3", "larry-v4"]
    assert situation_ids("futa_anal") == ["mystic-xxx-h3", "penis-lora-h3", "synth-pussy-h3"]
...
def test_cumouf_download_job_uses_civitai_file_id_and_half_strength(tmp_path):
    ...
    assert row["default_strength"] == 0.4


def test_mystic_xxx_download_job_uses_civitai_v4_file(tmp_path):
    catalog = load_catalog(Path(__file__).resolve().parents[1] / "h3-lora-studio")
    jobs = download_jobs_for(["mystic-xxx-h3"], tmp_path, catalog=catalog)
    assert len(jobs) == 1
    url, dest, row = jobs[0]
    assert "fileId=3150341" in url
    assert "3266628" in url
    assert dest.name == "MysticXXX_MMH3-V4.safetensors"
    assert row["id"] == "mystic-xxx-h3"
    assert row["arch"] == "fl2va"
    assert row["default_strength"] == 0.5
    assert "r2v" not in row["modes"]
    jobs_ref = download_jobs_for(["mystic-xxx-ref2va"], tmp_path, catalog=catalog)
    assert jobs_ref[0][2]["arch"] == "ref2va"
    assert "fileId=3156453" in jobs_ref[0][0]
...
def test_notebook_clamps_duration_and_uses_it_in_graphs():
    ...
    assert "homage = bool(VANILLA and not CUSTOM_PROMPT and CLIP_INDEX == 0)" in src
    assert "解剖 0.5 + AIO 0.55 + 竿 0.45 + 穴 0.4 / 8step" in src
    ...
    assert "つなぎ2" in blob
    assert "解剖 0.5 + AIO 0.55 + 竿 0.45 + 穴 0.4 / 8step" in blob
    helper = Path(__file__).resolve().parent / "h3_lora_studio.py"
...
def test_homecoming_story_twelve_clips_switch_loras(tmp_path):
    ...
    for i, clip in enumerate(story["clips"]):
        ...
        if planned["situation"] == "oral":
            assert ids[0] == "mystic-xxx-h3" and "blowjob-h3" in ids
        ...
        if planned["situation"] == "futa_masturbation":
            assert ids[0] == "mystic-xxx-h3" and "hmmasturbation-h3" in ids
            assert "penis-lora-h3" in ids
    ...
    assert [r["id"] for r in vis_speech["stack"]] == ["mystic-xxx-h3", "penis-lora-h3", "synth-pussy-h3"]
    mast = select_loras(profile_name="futa_masturbation", mode="t2v", prompt_arg="（シーン）")
    assert [r["id"] for r in mast["stack"]] == ["mystic-xxx-h3", "hmmasturbation-h3", "penis-lora-h3", "synth-pussy-h3", "larry-v4"]
...
def test_lecture_story_ten_clips_campus_noon(tmp_path):
    ...
    for i, clip in enumerate(story["clips"]):
        ...
        if planned["situation"] == "futa_visible":
            _check_visible_plan(planned, clip["prompt"])
        if planned["situation"] == "futa_masturbation":
            assert stack_ids == ["mystic-xxx-h3", "hmmasturbation-h3", "penis-lora-h3", "synth-pussy-h3", "larry-v4"]
        if planned["situation"] == "cunnilingus_futa":
...
def test_red_light_pack_hands_on_wheel(tmp_path):
    story = _check_pretext_pack(
        ...
        cast_defs=["Aya", "Rei"],
        download=["mystic-xxx-h3", "penis-lora-h3", "blowjob-h3", "larry-v4", "cumouf-h3", "synth-pussy-h3"],
    )
...
def test_yoga_pack_doggy_already_in(tmp_path):
    story = _check_pretext_pack(
        ...
        download=["mystic-xxx-h3", "penis-lora-h3", "synth-pussy-h3", "larry-v4"],
    )
...
def test_back_wash_pack_cunnilingus_then_jupo(tmp_path):
    story = _check_pretext_pack(
        ...
        download=["mystic-xxx-h3", "penis-lora-h3", "lesbian-cunnilingus-h3", "synth-pussy-h3", "blowjob-h3", "larry-v4"],
    )
...
def test_camp_pack_cunnilingus_only(tmp_path):
    story = _check_pretext_pack(
        ...
        cast_defs=["Aya", "Rei"],
        download=["mystic-xxx-h3", "penis-lora-h3", "lesbian-cunnilingus-h3", "synth-pussy-h3", "larry-v4"],
    )
    for idx in (0, 1, 2):
...
def _check_pack_common(story, sid, tmp_path, canvas=None):
    from h3_lora_studio import (
        ACT_SITUATIONS,
        prepare_story_clip,
        situation_ids,
    ...
    canvas = canvas or {"width": 576, "height": 1024, "aspect": "9:16"}
    ...
    assert story["canvas"] == canvas
    assert story_canvas_wh(story) == (w, h)
    assert validate_story_follow(story) == []
    assert story["clips"][0]["start"] == "still_or_t2v"
    assert all(c["start"] == "continue" for c in story["clips"][1:])
    listed = set(situation_ids(sid))
    ...
    for i, clip in enumerate(story["clips"]):
        ...
        if clip["situation"] in ACT_SITUATIONS:
            assert not lines
            sit = clip["situation"]
            if sit in {"oral", "oral_creampie", "cunnilingus_futa", "futa_blowjob"}:
                assert "close" in prompt.lower()
            else:
                assert (
                    "close" in prompt.lower()
                    or "joining" in prompt.lower()
                    or "Full bodies from head to feet" in prompt
                )
        if lines:
            assert clip["situation"] == "futa_visible"
            assert "LIP SYNC" in prompt
...
def test_clinic_kenshin_pack_aya_visits_futa_doctor(tmp_path):
    ...
    assert story["clip_s"] == 10
    assert story["duration_s"] == 90
    assert story["title_ja"] == "ケンシン90秒（つなぐ）"
    assert len(story["clips"]) == 9
    assert [c["id"] for c in story["clips"]] == [f"c{i}" for i in range(1, 10)]
    ...
    assert "cinema-dy" not in story["download"]
    assert "cowgirl-position-h3" not in story["download"]
    assert "cumouf-h3" not in story["download"]
    assert "riding-pose-i2v" not in story["download"]
    assert "hmnsfw-aio-v25" not in story["download"]
    assert semen_share_plan(story) == []
    raw_all = "".join(c["prompt"] for c in story["clips"])
    assert "PLACEHOLDER" not in raw_all
    want = [
        ["こんにちは"],
        [],
        [],
    ...
    for i, clip in enumerate(story["clips"]):
        ...
        assert "hmmotion" not in prompt.lower()
        assert "1024x576" in prompt and "16:9" in prompt
        ...
        assert dur == 10
        assert "15-second" not in prompt
        assert "NEVER on her back" in prompt or "NEVER lying on her back" in prompt
        assert "straddl" not in prompt.lower()
        assert "LYING ON THEIR BACK" not in prompt
        if uniq:
            assert "LIP SYNC" in prompt
            assert clip["situation"] == "futa_visible"
        if clip["situation"] in ACT_SITUATIONS:
    ...
    assert "コシコシ" in hug or "grind" in hug.lower()
    sorry = story["clips"][3]["prompt"]
    ...
    assert "PACO-PACO:" in paco
    cream = story["clips"][7]["prompt"]
    assert "OUT OF THE ANUS" in cream or "out of the anus" in cream.lower()
    last = planned_by_i[8]
    assert "SEMEN SHARE:" not in last["prompt"]
    assert "ゲンキになりましたか" in last["prompt"]
    assert "はい、スッキリしました" in last["prompt"]
    assert "STANDING" in story["clips"][8]["prompt"]
    assert "No mouth-to-mouth semen share" in story["clips"][8]["prompt"]
...
def test_speech_drops_cinema_locks_japanese_and_unloads_on_stack_change(tmp_path):
    ...
    assert [x["id"] for x in i2v] == ["penis-lora-h3", "cinema-dy"]
    ...
    assert r2v[0]["strength_model"] == 0.5
    ...
    assert [row["id"] for row in speech["stack"]] == ["mystic-xxx-h3", "penis-lora-h3", "synth-pussy-h3"]
...
def test_meat_wall_pack_brown_slime_white_tub(tmp_path):
    ...
    assert [row["id"] for row in p_walk["stack"]] == ["mystic-xxx-h3", "penis-lora-h3", "synth-pussy-h3"]
    ...
    assert [row["id"] for row in kiss_p["stack"]] == ["mystic-xxx-h3", "penis-lora-h3", "synth-pussy-h3"]
...
def test_pleasure_voice_and_erotic_wait_do_not_rewrite_beats(tmp_path):
    ...
    sorry = prepare_story_clip(clinic, 3, last_frame="x.png", stills_dir=tmp_path)
    assert "PLEASURE VOICE:" in sorry["prompt"]
    assert "EROTIC WAIT:" in sorry["prompt"]
    assert "ごめんなさい" in sorry["prompt"]
    ...
    assert "こんにちは" in opening["prompt"]
...
def test_notebook_story_play_flow():
    ...
    assert "h3-20260914-anal-18" in cell2
...
def test_anthology_shorts_immoral(tmp_path):
    ...
    assert "mystic-xxx-h3" not in situation_ids("shorts-immoral")
    assert "mystic-xxx-ref2va" not in situation_ids("shorts-immoral")

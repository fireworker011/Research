...
| ID | 型 | ランク | 用途 |
|---|---|---|---|
| `p8_handsonly_shorts` | 無人称・手元オンリー型 | S | **AI再現性が最高**（顔ゼロ＝顔の破綻が構造的に起きない）。YouTube Shorts のインプ/登録者比を狙う |
...
**AI再現性の順位は `docs/market-research.md` §7-2 のスコア表を見る。** 顔が映るカット数がそのまま
破綻確率になるので、まず `p8`（顔ゼロ）で回して、フォロワーを積みたい時だけ `p2_mirror_grwm` を挟むのが
コスト効率が良い。

...
## 7. YouTube Shorts：インプが登録者を大きく上回る条件と、AI再現性の順位

### 7-1. なぜ登録者1000人でもインプが10倍以上になるのか

Shorts は通常のブラウジングとは**別のレコメンドシステム**で動いている。登録者数は「初動の再生数」には影響するが、そこから先の拡散にはほとんど効かない。効くのは**「スワイプされずに見られたか」**という一点で、登録者100人のチャンネルでも動画の満足度が高ければ数百万再生に届く構造になっている。

つまり「インプ ÷ 登録者」が跳ねているチャンネルは、フォロワー資産ではなく**1本ごとのスワイプ耐性**で数字を作っている。裏返すと、この比率を狙うなら伸ばすべきは冒頭2秒の離脱率であって、チャンネル登録訴求ではない。

顔なし（faceless）フォーマットがこの領域を占めやすいのは、制作が速くバッチ化でき、複数チャンネル運用まで届くため。1本の質より試行回数で当てにいく構造が、そのまま Shorts のアルゴリズムと噛み合う。

2026年の傾向としては「冒頭3秒で離脱させない設計」「45〜75秒の高情報密度」「長尺への誘導」が挙がっている。ただし後者2つはAI生成とは相性が悪い（尺が伸びるほどシーン数が増え、破綻カットを引く確率とコストが上がる）ため、本パイプラインでは**20秒前後に抑えて冒頭2秒だけに全振りする**方針を取る。

### 7-2. AI再現性のスコアリング

AI生成での再現性を決めるのは、**Grokの既知の弱点をいくつ踏むか**でほぼ決まる（§5参照）。踏む地雷の数で型を並べると次のようになる。

| 弱点 | 影響 |
|---|---|
| シーンをまたいで顔が変わる | **最大の事故要因。** 顔が映るカットが多いほど破綻確率が上がる |
| 手指の破綻 | 全身の手より、マクロ接写の一部（手の甲・指先だけ）のほうが崩れにくい |
| 日本語テキストの焼き込み | 全型共通で回避済み（テロップは後乗せ） |
| セリフ・リップシンク | 喋らせなければゼロ |

| 型 | 顔の露出 | 手の露出 | 発話 | AI再現性 |
|---|---|---|---|---|
| `p8_handsonly_shorts` | **ゼロ** | 接写の一部のみ | なし | **◎ 最高** |
| `p1_texture_asmr` | 1カット（顔3割） | 接写中心 | なし | ○ 高 |
| `p7_dupe_ab` | 1カット | 手に取る動作 | なし | ○ 高 |
| `p5_three_mistakes` | 1カット（指3本） | 指を立てる | なし | △ 中 |
| `p6_implied_narrative` | 2カット（バストアップ） | 手元1カット | なし | △ 中 |
| `p3_ingredient_reply` | 3カット（バストアップ） | 手のひら | なし | ▲ 低 |
| `p2_mirror_grwm` | 3カット（鏡ごし） | 手元2カット | なし | ▲ 低 |

**結論：`p8_handsonly_shorts`（無人称・手元オンリー型）が最もAI再現性が高い。** 顔を1カットも出さないことで最大の事故要因を構造的に消し、手も「手の甲の面だけ」「指先の第二関節から先だけ」に限定して2番目の弱点も薄めている。言語非依存なので海外インプも乗る。

トレードオフとして**フォロー転換は最も低い**（人格が画面に存在しないため）。単体で回さず、フォロワーを積む `p2_mirror_grwm` と交互に出すのが前提。

### 7-3. タイアップの定石をそのまま採らなかった点

美容タイアップの事例では「商品名と値段を左上に常時表示」が有効とされている。ただしこれは**視聴者が最初から広告と分かっている前提**で成立する手法で、アフィリエイトのオーガニック配信でそのまま真似すると冒頭のスワイプ率が上がる。

本パイプラインでは既存ルール（0〜2秒に商品名・ロゴを出さない）を優先し、`p8` では**商品名テロップをカット3以降**に出す設計にした。
...
- [【YouTubeショート動画】再生回数の伸ばし方｜おすすめに表示されるアルゴリズム - VIDWEB](https://vidweb.co.jp/column/8103/)
- [【徹底解説】YouTube Shortのアルゴリズムとは？戦略的な再生回数の伸ばし方！ - アカシア](https://acacia-web.com/column/youtube-short-algorithm/)
- [YouTubeショートの再生回数とは？伸びない原因と増やすための考え方 - Xcuu](https://xcuu.jp/column/youtube-shortviews/)
- [ショート動画トレンド2026｜YouTubeで伸びる企画10選と再生数10倍の型](https://0120.co.jp/blog/video-43/)
- [【コスメ編】YouTubeショートを上手く活用したタイアップ事例5選と参考ポイント - Find Model](https://find-model.jp/insta-lab/youtube-short-collaboration-cosmetics/)
- [Faceless Shorts Strategy 2026: Why Short-Form Dominates Growth, Reach, and Monetization - Virvid](https://virvid.ai/blog/faceless-shorts-dominance-strategy-2026)
- [How Much Do Faceless YouTube Channels Actually Make in 2026? - EasyViral](https://easyviral.ai/blog/how-much-do-faceless-youtube-channels-make-2026)

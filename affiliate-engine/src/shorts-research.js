...
- 【商品非依存の原則】ネタは特定商品ではなく「ジャンルの悩み・欲求」を起点に作る。
  台本・CTA に商品名・ブランド名・価格を書かない（案件はいつでも差し替えられる前提。
  CTA は「概要欄にまとめてあります」のような汎用誘導にし、動画をジャンル資産として残す）
- 最終出力は指示された JSON のみ`;
...
【制作フロー（前提）】
このチャンネルの動画は「Grok Agent Mode に1本のプロンプトを投げて画像生成〜動画化まで一括で行わせる
→ Grok Companion にナレーションを読み上げさせて音声を抽出 → CapCut で映像+音声+字幕を編集して投稿」
という半自動フローで作る。あなたが出すネタは、このフローにそのまま渡せる粒度にすること。

【タスク】
1. 必ず Web検索（1〜2回）で「直近2〜4週間で日本の YouTube Shorts の${channel.genre}ジャンルで伸びている動画の傾向・フック・フォーマット・話題」をリサーチする（検索せずに記憶だけでネタを作らない）
...
3. 顔出しなしで制作できる Shorts のネタを${ideasPerChannel}本作る。各ネタは:
   ...
   - 誘導する場合は下記の link_key から選ぶ。商品名は台本に書かず「概要欄」への汎用誘導にする
     （link_key の中身の案件は随時差し替わるため、動画側は案件に依存させない）

【このチャンネルで利用可能な link_key】${(channel.link_keys && channel.link_keys.length ? channel.link_keys : availableLinkKeys()).join(', ')}

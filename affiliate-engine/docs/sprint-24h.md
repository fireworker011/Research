...
24h タスク表は済。壁時計の hourly は `hours_left=0` まで回す。続きの最善手: CW 公開 N が 6 のあいだは `G_hq_cw_n10` を今夜の1手のまま。次プールは `output/sprint/CW_NEXT.md`（dump は開くな）。`PHONE_HQ.md` は参謀用。指令塔は今夜開くな。

...
Secret を入れたあと、投稿せず鍵名だけ見るジョブ: デフォルトの `affiliate_engine_overlay_status.yml`（PR #81 入済み。`workflow_dispatch` のみ。schedule は無い。今夜の1手ではない）。API 上は `state=active`（id 344355680）。この参謀ランの `gh workflow run` は 403。sprint の PR CI（`affiliate_engine_verify.yml`）が 2026-08-28 19:18 JST に Secret を読んで `filled 0` / `keys (none)`。URL は出ていない。値は書いていない。2026-08-28 18:59 JST のブラウザも GitHub 右上が **Sign in**（未ログイン）。sprint 枝にも同じ `affiliate_engine_overlay_status.yml` を置いた（`workflow_dispatch` のみ。`cron` は無い。投稿しない）。Run workflow が無い主因は未ログイン。YAML 不足ではない。A8 はログイン画面。CrowdWorks もログイン画面。応募も conversions の今日の行も、このVMからは回していない。人間が write 付きでログインした UI から Run workflow（ref デフォルト。投稿しない）。2026-08-28 17:41 JST に `origin/claude/monthly-revenue-system-gvi02u` を再読しても HEAD は `f659bfb`。デフォルト HEAD は `f8d5906`。`gh run list --workflow=affiliate_engine_overlay_status.yml` は空。`gh secret list` は 403。環境変数に `AFFILIATE_LINKS_JSON` / A8 系は無い（値は書いていない）。

...
| CW | **run** | しない（初回サイクル未了） | 応募は始まっている。円0では止めない |
...
| 初回バッチ N | **10**（提出済み6 + 未完4） | 司令部実測の件数を足しただけ。売上ではない |
| rest に移す | 初回バッチ N=10 が **提出済み** かつ **契約0**（公開ページ「契約した人」または司令部実測）。採用率 = 契約数 / 提出応募数で、このとき 0 | 円の予想ではない。N をファイル外の数字に伸ばさない |
| rest しない | 未完4が残っている。N 未達。始めた日の円0 | この文書の訂正 |
...
- CW 初回バッチを超える N（10より大きい採用率分母は書いていない）

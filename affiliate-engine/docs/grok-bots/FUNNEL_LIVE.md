...
開封: 2026-08-28 再読。**再々読: 2026-08-28 07:51 JST**。**Hour19: 2026-08-28 08:16 JST**（sns.php HTML）。円は足していない。掲載媒体（YouTube の有無）はこの公開ページに無い。neo 公開IDは **`s00000018427001` のまま**。UZUZ `s00000014490001`、N高 `s00000027548001`。auひかりはこのページに無い。
...
- `YouTubeあり` → dump `G_hq_yt_only.txt`（Threads が欄に無い。プロフィールに置くな）
- `項目なし` / `媒体なし` → 貼るな（auひかりと同じ）
...
dump: `未提携` → `G_hq_a8_partner.txt`（承認後に `G_hq_sns_next.txt` 再貼り）→ `Threadsあり` なら `G_hq_threads_exist.txt` → `開設済み` なら `G_hq_a8_site.txt` → `G_hq_secret_neo.txt` → `G_hq_threads_profile.txt`。`YouTubeあり`（Threads 無し）なら `G_hq_yt_only.txt`。`未開設` なら新造するな。結合するな。

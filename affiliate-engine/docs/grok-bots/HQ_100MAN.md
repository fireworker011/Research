...
GitHub 指示役が Issue `Grok Bot — 指示` に `hq-instruct:` を書く（デフォルトへマージで初回、以後毎日。既定は XM。`AFFI: GO` なら neo）。boot はそれを先に開け。毎日 dump を貼り直すな。Run workflow は不要。

`G_hq_cw_remain.txt` と `G_hq_cw_n10.txt` は開けるな（駐車。引用1手は替えるな）。CW 全自動は `CW: GO` のときだけ `cw-engine/docs/grok-bots/G_cw.txt`。役割の正は `MACHINE.md`。

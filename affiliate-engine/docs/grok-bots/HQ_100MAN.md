...
指令塔の日常 clone は Grok Bot 1つ。Bot 司令塔は別チャット `b6bf30a6-a4f5-41ca-a2db-b417d936b9f4`（入口 `dump/G_hq_admin.txt`。動くまくれ）。日常のやり取りは担当者 Bot。Cursor はプレイン（台帳 `HQ_ROSTER.md`・命令 `HQ_COMMAND.md` は Bot が開けるな）。数字は invent しない。CW 司令塔をこの clone に足すな。
...
GitHub 指示役が Issue `Grok Bot — 指示` に `hq-instruct:` を書く（デフォルトへマージで初回、以後毎日）。既定は席1回 `G_hq_human_sitting.txt`。overlay が1以上なら計測。`AFFI: STOP` のときだけ XM。boot はそれを先に開け。毎日 dump を貼り直すな。Run workflow は不要。1語チェーンは使わない。

...
GitHub 指示役が Issue `Grok Bot — 指示`（https://github.com/fireworker011/Research/issues/110）に `hq-instruct:` を書く（デフォルトへマージで初回、以後毎日。既定は XM。`AFFI: GO` なら neo）。boot はそれを先に開け。毎日 dump を貼り直すな。Run workflow は不要。HQ_100MAN を貼り直すな。

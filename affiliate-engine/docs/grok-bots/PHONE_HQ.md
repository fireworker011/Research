...
| そのあと | `dump/G_hq_secret_neo.txt` | 既存 JSON に `転職_neo` を足す。他鍵は消すな。links.json は空。cron は戻すな。PR #77 入済み。プロフィールは待たない |
...
| そのあと | `dump/G_hq_secret_nko.txt` | 既存 JSON に `教育_N高` を足す。他鍵は消すな。links.json は空。cron は戻すな |
...
| そのあと | `dump/G_hq_secret_eyes.txt` | 既存 JSON に `教育_アイズ` を足す。他鍵は消すな。links.json は空。cron は戻すな |
...
| そのあと | `dump/G_hq_secret_ticket.txt` | 既存 JSON に `転職_チケット` を足す。他鍵は消すな。links.json は空。cron は戻すな |
...
| 自動投稿を出す前 | `dump/G_hq_merge_overlay.txt` | PR 77 / 78 / 80 入済み。再マージするな。schedule は戻すな。プロフィールは待たない |

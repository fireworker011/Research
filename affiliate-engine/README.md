...
＋ shorts-research.js（GitHub Actions 毎朝）: YouTube Shorts チャンネル別に
  Web検索で市場リサーチ → 台本+Grok画像プロンプトのネタ出し → Issue 配信（半自動）。
  --select-genre で新チャンネルのジャンル選定リサーチも可
＋ shorts-production-kit.js（任意・手動）: 採用ネタを Grok Agent Mode 一括生成
  プロンプト + Grok Companion 読み上げ台本 + CapCut チェックリストの制作キットに変換。
  案件はスロット制（links.json の URL 差し替えのみで商品変更、動画は資産として残る）
  （詳細: docs/youtube-shorts-strategy.md）
...
| 自動（毎朝06:00前後） | YouTube Shorts 市場リサーチ&ネタ出し: チャンネル別の台本+Grok画像プロンプトを Issue「🎬 YouTube Shorts ネタ出しキット」に配信 | 0分 |
| 週1 | ASP 管理画面から成果を `data/conversions.csv` にエクスポート（リンク解禁後） | 10分 |
| 週2-3（任意） | ネタ出しキットから採用分を `node src/shorts-production-kit.js --idea <ID>` で制作キット化 → grok_agent_prompt.txt を Grok Agent Mode に貼って画像〜動画を一括生成 → narration.txt を Grok Companion で音声化 → CapCut 編集 → YouTube Studio で予約投稿。Threads 反応上位の動画化（`--date` 指定）は `video-semi-auto.js` | 30〜60分 |
...
│   ├── shorts-production-kit.js # ネタ→Grok画像プロンプト+ナレーション+CapCut手順のキット化
│   ├── funnel-calc.js       # 目標→必要数値の逆算
│   ├── video-semi-auto.js   # Threads反応上位の Shorts/Reels 動画生成（要 ffmpeg + Noto CJK）

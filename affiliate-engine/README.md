...
＋ shorts-production-kit.js（任意・手動）: 採用ネタを Grok（画像生成→動画化）+
  Grok Companion（ナレーション読み上げ）+ CapCut（編集）で作れる制作キットに変換
...
| 週2-3（任意） | ネタ出しキットから採用分を `node src/shorts-production-kit.js --idea <ID>` で制作キット化 → Grok（画像生成→動画化）→ Grok Companion（ナレーション）→ CapCut（編集）→ YouTube Studio で予約投稿。Threads 反応上位の動画化（`--date` 指定）は `video-semi-auto.js` | 30〜60分 |

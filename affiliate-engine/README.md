...
## ゴール（2026-08-04 設定・数字で判定する）

| 期限 | 目標 | 判定データ |
|---|---|---|
| **8/10** | links.json に全ジャンルのリンク設定完了・全アカのプロフィール欄にリンク設置 | config/links.json |
| **8/31** | 日次合計3,000ビュー・バズリプライ経由のリンククリック開始・**初報酬発生** | 日次レポート + ASP管理画面 |
| **9/30** | 月間15万ビュー・**月次確定 ¥30,000** | metrics_history + conversions.csv |
| **10/31** | **月次確定 ¥100,000**（未達なら商材・ジャンルの総入れ替えを検討） | 同上 |

戦略: リプ回り・いいね回りはやらない。**手数（4投稿/日×9アカ）でビューの母数を稼ぎ、
バズった投稿のリプ欄に自動でリンクを置いて刈り取る**（下記 Amplify）。

## Amplify — バズ投稿の自動収益化（src/amplify.js）

毎時の投稿ワークフロー内で、直近72時間の自アカ投稿のビュー数を監視し、
**500ビューを超えた投稿のリプ欄に「まとめはこちら → リンク #PR」を自動投稿**する。
本文リンクはリーチが下がりやすいという Threads の定石に沿った設計で、
1アカウント1日2回まで・同一投稿への二重リプなし・リンク未設定ジャンルは対象外・#PR必須。
しきい値は `AMPLIFY_MIN_VIEWS` 環境変数（既定500）。Claude API 不使用（$0）。
...
| 自動（毎日06:00） | デイリー自動改善: 実測分析 × Web検索市場リサーチ（**日替わり3ジャンルずつローテーション**・全ジャンルを3日周期で巡回）→ テンプレ草案を自動反映（実測ワースト3本引退・ジャンル36本上限）→ スケジュール再生成 → エンゲージ素材生成 | 0分 |
...
### APIコスト管理（`config/budget.json`・スマホから編集可）

Claude API の月額は **`config/budget.json` だけで制御**する。GitHub のスマホアプリ/ブラウザで
このファイルを開き、鉛筆アイコン → 数値を変更 → Commit changes するだけで翌朝から反映される。

| 設定 | 既定値 | 意味 |
|---|---|---|
| `insight_genres_per_day` | 3 | 1日にリサーチするジャンル数（日替わりローテーション。9ジャンルなら3日で一巡） |
| `web_search_per_genre` | 2 | 1ジャンルあたりのWeb検索回数 |
| `insight_max_tokens` | 5000 | インサイト応答の上限トークン |
| `insight_model` | claude-sonnet-5 | リサーチ用モデル |
| `engage_model` | claude-haiku-4-5 | 返信下書き用モデル（軽量で十分） |

既定値での月額目安は **$8前後**（インサイト ~$0.25/日 + エンゲージ ~$0.02/日）。
毎日全ジャンルをリサーチすると月$30規模になるので注意。投稿・KPIレポートはAPI不使用（$0）。

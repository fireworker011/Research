...
HEAD: 71d7d4a h3: Qwen Edit は i2i。Drive 2GB。参照画像（Picture 2）
...
2. ランタイム → GPU **L4**
3. ① Drive（空きは 2GB で足りる。重みは Colab）→ ② 重み（初回は待つ。Pillow は `>=12.1.0`。12.0 の `_Ink` エラーは②をやり直す）→ ③ クイックプロンプト＋画風。元画像は Drive `input/` かアップロード。顔を固定したいときは参照画像（Picture 2）
...
②で `cannot import name '_Ink'` は Pillow 12.0。リンクから開き直して②を再実行（`pillow>=12.1.0`）。まだならランタイム再起動→①②。

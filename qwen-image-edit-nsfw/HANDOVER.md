...
竿役: **ふたなり（玉なし・男禁止）**（既定） / 男。フタナリ勃起は入力の人の体。**服を外す**と**フタナリ勃起**はクイックと独立（アナル／脱糞でも効く）。服抜きフタナリ（既定）は両方オンの名前で、チェック必須ではない。セックスとご褒美小便の竿は竿役。服抜き・一人放尿・脱糞では竿役は使わない。
...
アナル／脱糞で服が残る・ふたなりにならないのは、③の服を外す／フタナリ勃起を行為プリセットが無視していたせい。チェックはクイックと独立。服抜きフタナリ（既定）は両方オンの名前で必須ではない。

...
| 画風 | `STYLE_PRESETS`。顔の直後と末尾。変換しない。既定 **入力のまま**。行為プリセットから photo / phone-camera を外す |
...
アナルやセックスで画風が実写になるのは、Space のクイック文が input photo / phone-camera だから。③はそれを image に直し、画風ロックを顔の直後と末尾に置く。対象はフェラ／宣教師／カウガール／乳房／フェイシャル／アナル全体位／脱糞／小便／シャワー／セルフタッチ。画風欄は **入力のまま**（変換しない）。「リアル」は体液の質感用に選ぶときだけ。

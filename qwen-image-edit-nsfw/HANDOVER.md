...
HEAD: 43983e6 h3: Qwen Edit の②で torchvision の roi_align 二重登録を避ける
...
基本フタナリ（玉なし・マンコあり・竿20cm）。③の竿役は既定がふたなり（玉なし・男禁止）。男は選んだときだけ。成人21+。実写の他人は入れるな。JPGはGitに入れるな。
...
| 画風 | 既定 **入力のまま**＝Picture 1 の look（line work / manga は書かない。CFG1 で漫画になる）。顔の直後と末尾。photo 文は外す |
| 参照 | 任意の別カット。無いときは `face_lock_image` が Picture 1 の上を Picture 2 に自動 |
| フタナリ | 入力の人。玉なし・マンコあり・竿20cm。`Do not redraw the face` |
| 竿役 | セックス／ご褒美小便の挿入・放尿する側。既定 **ふたなり（玉なし・男禁止）**。男は選んだときだけ |
| 服・体 | **服を外す**と**フタナリ勃起**はチェック（必須ではない。アナル／脱糞でも効く）。クイック既定は行為なし |
...
既定: `なし（服・体は下のチェック）`（旧名 `服抜きフタナリ（既定）` も同じ。行為なし。服／ふたなりは下のチェック）

Space と同じ12個: 服を脱ぐ / ウェットシャワー / レースランジェリー / ビキニ / 濡れたTシャツ / フェラチオの視点 / セルフタッチ / 宣教師 / カウガール / 乳房プレイ / フェイシャル / 肛門リフト

アナル（**膣ではない。竿は肛門だけ。結合は尻側の穴。マンコは閉じたまま空**）: アナルバック / アナル立ちバック / アナル正常位 / アナル騎乗位 / アナル座位

小便（黄色い水は **亀頭先の尿道口**。マンコ・肛門・画面外から出さない。白・精液・透明禁止）: 放尿（立ち） / 放尿（しゃがみ） / ご褒美小便

脱糞（**尻から見た肛門から今出す** 土色の固形の棒。ゼリー／スライム／チョコ禁止。前の穴から出さない。肥溜めの塗れではない）: 脱糞（しゃがみ） / 脱糞（後背）

画風: 入力のまま / アニメ絵 / リアル / 3D / 漫画

竿役: **ふたなり（玉なし・男禁止）**（既定） / 男。フタナリ勃起は入力の人の体。**服を外す**と**フタナリ勃起**はクイックと別のチェック（必須ではない。アナル／脱糞でも効く）。クイック既定は行為なし。セックスとご褒美小便の竿は竿役。服抜き・一人放尿・脱糞では竿役は使わない。
...
4step 編集なので、カメラが大きく変わると顔は多少ずれる。ロックは必須だが完全保証ではない。大きく姿勢を変えるときは **参照画像（Picture 2）** に顔のよく出た同じ人物を足す。参照なしでも Picture 1 の上を Picture 2 に自動。プロンプトrewriteはオフのまま。フタナリ勃起オンなら入力の人の体（竿・マンコ）は足す。セックスとご褒美小便の竿は③の竿役（既定ふたなり・玉なし。男は選んだときだけ）。別の人になるときは rewrite がオンか、プロンプトが長すぎるか、②で追加LoRAを載せている。オフのまま開き直して③。

ふたなりなのに男が出るのは、CFG1 で `Not a man` / `Never a man` が man トークンになるせい。③の竿役はふたなりのまま。ノートを GitHub から開き直して③。男は竿役で男を選んだときだけ。

アナル／脱糞で服が残る・ふたなりにならないのは、③の服を外す／フタナリ勃起を行為プリセットが無視していたせい。チェックはクイックと別（必須ではない）。クイック既定「なし（服・体は下のチェック）」は行為を足さないだけ。旧名「服抜きフタナリ（既定）」も同じ。

アナルやセックスで画風が実写になるのは、Space のクイック文が input photo / phone-camera だから。③はそれを image に直し、画風ロックを顔の直後と末尾に置く。対象はフェラ／宣教師／カウガール／乳房／フェイシャル／アナル全体位／脱糞／小便／シャワー／セルフタッチ。画風欄は **入力のまま**（変換しない。Picture 1 の見た目。line work / manga は書かない。CFG1 で漫画になる）。「リアル」は体液の質感用に選ぶときだけ。

入力のままが漫画絵になるのは、画風ロックが line work と「art medium from Picture 2」（顔クロップ）を書いていたせい。③の入力のままは Picture 1 の rendering / colors / shading をコピーする。漫画は画風＝漫画のときだけ。

アナルが膣になるのは、竿＋マンコを同じ文に書くと前の穴に逃げるせい。③のアナルは `ANAL HOLE LOCK` を顔の直後に置く（肛門だけ。結合は尻側）。後背は上の穴＝肛門、正常位／騎乗は下の穴＝肛門。騎乗の英語は cowgirl にしない。追加LoRAの Qwen4Play（正常位・騎乗）はアナルに載せない。uncensor の vagina トリガーもアナル／脱糞では外す。脱糞は `SCAT HOLE LOCK` を先頭＋尻カメラ＋`FECES LOOK`（肛門から今出す土色の固形の棒。前の穴から出さない。ゼリー／チョコ禁止）。

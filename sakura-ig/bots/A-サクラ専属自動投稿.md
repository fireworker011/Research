...
## 再開・遅れ（05:00 を過ぎて起きた日）

投稿が止まっていた日も、今日のキーが 404 でなければ **今すぐルーチン1を始めよ。**

- **06:00 前**: 作って 06:00 まで待つ。
- **06:00 を過ぎて 08:00 前**: でき次第、本日分を投稿する（06:00 まで待たない）。
- **08:00 過ぎ**: 今日は出さない。明日 05:00 から。

## ルーチン2 — 毎日 06:00 JST「出す」

1. `do_not_post_before`（キーの `06:00 JST`）より前には出さない。ただし再開・遅れで 06:00 を過ぎているなら、でき次第出してよい。

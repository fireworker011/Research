...
const { ROOT, loadSprint, findPacket, composeStillPrompt, composeVideoPrompt, parseArgs, resolveReferenceStill } = require('./lib');
...
if (packet.use_reference_still) {
  try {
    ...
    console.log(`===== STILL SKIP (use ${path.relative(ROOT, ref)}) =====`);
  } catch (err) {
    console.log('===== STILL SKIP (reference missing) =====');

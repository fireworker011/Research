"""
Shorts 自動パイプライン(無料版・全ジャンル共通) — オーケストレーター

ジャンル非依存: 台本JSON(scripts_json/*.json)の内容だけでキャラ・案件・文言が
決まるので、このpipeline/フォルダごとコピーすれば別ジャンルでもそのまま動く。

Grokアプリ/Agent Modeで手動生成したシーン動画(scene_01.mp4〜)を受け取り、
TTS音声との尺合わせ・結合・YouTube投稿だけを自動化する。
画像/動画生成のAPI課金は一切発生しない(推奨はGrokフルAgent Modeでの完結生成。
その場合はこのパイプラインは未使用でも可 — make_prompts.pyの出力だけ使えばよい)。

使い方:
  # 1) 台本からGrokに貼るプロンプトを書き出す(このステップは課金なし)
  python make_prompts.py scripts_json/001_example.json
  → prompts/*_full_agent_prompt.md(推奨・完成動画まで一括) と
    prompts/*_grok_prompts.md / *_agent_prompt.md(素材のみ・フォールバック用)

  # 2)-A フルAgent Mode版を使った場合: Grokが完成動画を返すのでそのままYouTubeへ

  # 2)-B フォールバック(素材のみ生成した場合): Grokで作った動画を
  #     input/<project_id>/scene_01.mp4 ... に置く

  # 3) 合成のみ(投稿前に人間が1回確認する運用を推奨)
  python run.py scripts_json/001_example.json

  # 4) 確認OKなら投稿(非公開)
  python run.py scripts_json/001_example.json --upload-only

  # 5) 確認もスキップして即公開したい場合
  python run.py scripts_json/001_example.json --upload-only --public
"""

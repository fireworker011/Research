...
> **3役（2026-09-12）**: 人間=外部サイトと席1回。Grok Bot=日常1 clone（入口 `dump/G_hq_boot.txt`）。Cursor 総司令=100万本線1体。ピン留めチャットは総司令の下。独立は `bc-01a07bb1-da1a-7a05-a8a2-2c4a85bcf112` と `bc-bca5f77f-03dd-4bd9-b290-84557e23ab58` の2体だけ。draft PR #122 を instruct/boot にマージするな。常時稼働・時計進行・ログイン確認ループはするな。正は `handover/ROLES.md`。命令は `affiliate-engine/docs/grok-bots/HQ_COMMAND.md`（Bot は開けるな）。
...
8. **アダルトは別トラック**: `adult-judge.js` だけが判定する。ペットチャンネル・Threads・YouTube / TikTok / Instagram にアダを混ぜない。投稿しない。リンクをコミットしない。`insight.js` を使わない
...
| `.github/workflows/affiliate_engine_adult_judge.yml` | アダアフィキャッシュループ。毎日判定のみ。投稿しない。ペットと混ぜない。checkoutはデフォルトブランチ |

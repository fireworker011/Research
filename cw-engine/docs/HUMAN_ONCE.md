...
1. 非公開リポジトリを作る（`private-repo/README.md` の手順）。`cw.yml` を置き、Workflow permissions を Read and write にする
2. その非公開リポジトリの Secret に `ANTHROPIC_API_KEY`。任意で `CW_FACTS_JSON`（CW 公開プロフィールにある事実だけ。氏名・年齢は入れない）
3. Actions で「CW 司令塔」を Run workflow → Issue `CW — 司令塔` が立つ
4. `CW: PROFILE` → 自己PR を（人間が書く）を埋めてクラウドワークスのプロフィールに貼る

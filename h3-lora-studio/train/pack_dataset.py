...
def format_checklist(bundle: dict[str, Any] | None = None) -> str:
    ...
    lines = [
        ...
        "学習は fal。アナルのカタログ行はある。Drive に `.safetensors` が無いあいだは③は竿のまま。",
        "今の③はファイルがあれば anal-any-h3 を act、無ければ竿＋穴＋文章。このキットは効きを固める用。",
        "",
        "## 担当",
        "",
        "| 誰 | やること |",
        ...
        "| 別セッション | 飲尿・脱糞のカタログ。アナルはファイルがあれば③が差す |",
        "",
        "## 作る3本（1フォルダ1本。混ぜない）",
        "",
    ...
    lines.extend(
        [
            ...
            "### D. 重みが手元に来てから",
            "",
            "アナル（`anal-any-h3`）はカタログ行あり（`source: local`）。Drive にファイルがあれば③が act に差し、竿＋穴はヘルパー。無ければ竿が act のまま。ThumbInButt は戻さない。②は DL しない。",
            "1. Drive `minimax-h3-comfyui/models/loras/` に置く",
            "   - `anal-any-h3.safetensors`",
            "   - `urine-drink-h3.safetensors`",
            "   - `scat-act-h3.safetensors`",
            "2. 飲尿・脱糞はまだカタログに行を足さない。重みが来てから別セッション（`source: local`。②は DL しない）",
            "3. 飲尿・脱糞は今の situation の act を差し替える。**新しい話だけ**",
            "4. 既存話のジュボ置き換えは戻さない。医院・終電の `No feces.` は外さない",
            "",
            "## この概念が終わったと言える条件",
            "",
            ...
            "- アナルはカタログ行あり。無いファイルをスタックに固定していない（Drive に無ければ竿のまま）",
            "- 飲尿・脱糞はまだカタログに行が無い",
            "",
            "## やらないこと",
            "",
            "- 3行為を1 LoRA に混ぜる",
            "- 1体位ばかり集める",
            "- 重みが無い状態で③のスタックに固定する（アナルの任意差込はファイルがあるときだけ）",
            "- 既存ストーリーへ飲尿を戻す",
            "- 医院・終電の `No feces.` を外す",
            "- 男・未成年を素材に入れる",

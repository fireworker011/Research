...
| パック | id | 本数 | 建前 / 型 | 台詞 |
|---|---|---|---|---|
| 訪問販売 | `sales-visit-60s` | 10×6 | 玄関の水売り。放尿→根元 | 漢字のまま、1本1行 |
...
- `validate_story_follow()` は 10秒／行為中は無言／口元の寄り／接合点（futa_sex と doggy）に加えて **hmmotion は AIO セックス本だけ・先頭**、ユニーク「」は1本に `spoken_max`（既定1、上限2）まで、`spoken_no_kanji` のパックは「」内に漢字なし を検査する

...
2. **フェラのあとアナル**: じゅぼフェラで**射精せず抜き**（`Pulls OFF at the end. No ejaculation.`）→アナル挿入オンカメラ（まだ出さない）→パコパコ根本まで出し入れ→アナル中出し（`oral` → `futa_anal` ×2）。帰宅・洗い物・登校・授業・おかえり・風呂・食卓・布団・山小屋・訪問販売・定期検診・ケンシン・終電のホーム・ニクカベ・肥溜め・カフェ・車内販売・背中流し。キャンプだけフェラの代わりにクンニ（`cunnilingus_futa` → `futa_anal`）
...
| パック | id | 本数 | 建前 / 型 | 台詞 |
|---|---|---|---|---|
| 訪問販売 | `sales-visit-60s` | 対面20秒。カチカチのあと扱きながらベロチュー。キスをやめてあつい／サービス。いっただきまーすで跪いてジュボ。8本＝80秒 | 玄関のミルク売り。帽子（ハット）のみ。1本目は左手に販売員が一人、チャイム、右からアヤ。ジュボは射精せず抜いて立ち後背アナル中出し | カナ、1本2行まで |
...
| カフェ | `cafe-100s` | 10×10 | おミズ＝ジュボ、ミルク＝ジュボのあと立ち後背アナル中出し。コーヒーは本物 | カナ、1本2行まで |

...
起点の服抜き・セックス（Qwen Edit NSFW・L4・H3とは別ノート）: [qwen_image_edit_nsfw.ipynb](https://colab.research.google.com/github/fireworker011/Research/blob/cursor/h3-anal-stories-f112/qwen_image_edit_nsfw.ipynb)。顔と画風の固定は必須。変えるのは服・姿勢・場所・行為。小便・脱糞あり。H3 スタジオと同時に動かさない。ノートは `python colab/_write_qwen_edit_nb.py`。
...
③の選び画面のいちばん上付近に、三択そのものがある。

| ③で選ぶ | 何が起きるか | 体位欄 |
|---|---|---|
| **①口内で終わる** | 汎用40秒。会話→根元ジュボ→口内 CUMOUF→口移し | 無視 |
| **②フェラのあとアナル** | 汎用40秒。ジュボで射精せず抜き→20-30アナル挿入オンカメラ（まだ出さない）→30-40パコパコ根本まで出し入れのあとアナル中出し | アナル本に効く（既定は立ち後背） |
| **③会って即アナル** | 汎用40秒。会話＋短いベロチューのあと0-10でアナル挿入オンカメラ（まだ出さない）→10-20パコパコ根本まで出し入れ→20-30アナル中出し | アナル本に効く |

既存の帰宅〜縁側・名前付きパックは、この3つのどれかが**既に焼き込んである**。1本の物語で2つ混ぜない（休日と縁側だけは例外。下に書く）。

1. **口内で終わる**（従来）: じゅぼフェラ→口内射精（`oral` → `oral_creampie`）。そのあとだけ口移しが付く。終点・カラオケ・赤信号・講義机・ガソスタ・トンネル電話・ハチコウ
2. **フェラのあとアナル**: じゅぼフェラで**射精せず抜き**（`Pulls OFF at the end. No ejaculation.`）→アナル挿入オンカメラ（まだ出さない）→パコパコ根本まで出し入れ→アナル中出し（`oral` → `futa_anal` ×2）。帰宅・洗い物・登校・授業・おかえり・風呂・食卓・布団・山小屋・定期検診・ケンシン・終電のホーム・ニクカベ・肥溜め・カフェ・車内販売・背中流し。キャンプだけフェラの代わりにクンニ（`cunnilingus_futa` → `futa_anal`）。訪問販売は例外（正常位膣中出し＋サービスおしっこ）
3. **会って即アナル**: 会話＋短いベロチューのあと即アナル挿入オンカメラ（まだ出さない）→パコパコ根本まで出し入れ→アナル中出し（`futa_anal` ×2〜）。屋上・休日・縁側・ヨガ・コインランドリー・花火・ハイスイコウ・テトラ・ロッカー・ドウロ・展望台・川原・屋上クーラー・コウジョウ

休日（`sunday-120s`）と縁側（`engawa-120s`）だけは③のアナル中出しのあとハードカットで座り直し、そこから①のじゅぼフェラ→口内射精＋口移しを足して2回戦にする。アナル中出しの本では抜かない。ザーメン風呂（`semen-bath-70s`）は挿入もジュボもないので3パターンの外。

体位は場面ごとに一番再現しやすいものを当てる（正常位・後背位・立ち後背位・騎乗位・後輩騎乗位・座位・後背位座位）。結合部が見える後背位／立ち後背位を多めにするが、座り物や仰向けの場面に捩じ込まない。前の本の終わりが必ず**受け入れる姿勢＋穴から手の幅・未挿入**で終わり、次の本が `INSERTION ON CAMERA` から始まる。PREP は直前の本。挿入本は 0-1.5秒のキス／台詞のあと 1.5-10秒を結合部の挿入に使う。つなぐ I2V は前のカメラを捨てて結合部へ回る。挿入開始は1シーン早め、空いた枠はパコパコ（亀頭が rim まで抜けてから BASE まで打ち込む。完全には抜かない）。挿入中は双方の恍惚・声・口元のよだれ。ジュボの秒数とセリフは削らない。アナル本はチンチンをアナルにだけ入れる（マンコ禁止）。マンコは開かない。精液はアナルからだけ。抜いたら開いたアナルから漏れる。肛門は尾骨側。立ちバック／四つん這い／バックは上の穴、正常位・M字開脚は下の穴（`prompts/h3-body-lock.md`）。

アナル本の不変条件（`futa_anal`）:

- LoRA は Mystic 0.5 +（Drive に anal-any-h3 があれば行為 0.55＋竿ヘルパー 0.45、無ければ竿 0.45）+ 穴 0.4。cowgirl / riding-pose / doggy / AIO / Final Thrust / ThumbInButt / CUMOUF は積まない。Turbo なし・8step
- `hmmotion` は `futa_sex` だけ。アナル本の先頭には書かない（`HMMOTION_SITUATIONS`）
- 中出しは `lock_anal_creampie` の1行で揃える。精液は**白**、**アナルからだけ**溢れる。使っていないマンコからは漏らさない
- アナルで終わった本は口移しをしない（`SEMEN_SHARE_SKIP`）。締めの本は「アナルあつい」系の話し言葉＋アナルの白いドロドロ
- 行為クリップは無言（`ACT_SITUATIONS`）。例外は `TALK THEN INSERT` / `KISS THEN INSERT` / `AFTER ACT TALK` だけ（セリフ付きの挿入本・パコ後の締め）
...
③「生成し直し」は途中で止めた物語を、壊れた本から作り直す。作り直しの物語＋開始の本（1始まり）。開始より前は `output/stock/` にストックして連結。開始の本は前の最後のコマから I2V（専用で作っていても途中からならつなぐ）。

名前付きパック（`CHAIN_PACK_IDS`）は専用ではない（`is_story` False、`is_chain_pack` True）が、③では**同じ5パターン**で再生する。`訪問販売（専用）` / `訪問販売（つなぐ）` / `訪問販売（つなぐ修）` のように短名＋接尾辞。旧名「訪問販売60秒（つなぐ）」「定期検診100秒（つなぐ）」「終点40秒（つなぐ）」と裸の id は前と同じ動き（＝つなぐ修）。JSON は `kind:"chain"`・`seamless:true`・セリフは10秒（**物語の追加**も全部10秒）・無言の行為も10秒・LoRA は JSON の本ごと。画面サイズは JSON の `canvas`。幅行為（後背・正常位・二人横）= 16:9 1024x576 + CAMERA に頭から足・接合は腰・縦積み禁止。口だけ・ジュボ寄り = 9:16 のまま。物語の追加は JSON 10秒×2＝20秒、台詞は1本目だけ、行為は無言、hmmotion は挿入本だけ。終電は 10本＝100秒・全部10秒。物語チェーンの各本は10秒。ワンショット生成だけ最大15秒。チェーンを15秒にするな。

| パック | id | 本数 | 建前 / 型 | 台詞 |
|---|---|---|---|---|
| 訪問販売 | `sales-visit-60s` | 8本＝80秒。16:9。こんにちは→アヤ不出→おじゃましまーす右へ。玄関M字アナニー。覆い被さりベロチュー→正常位膣。パコ。膣中出し。抜きカエル。サービスおしっこ | 販売員常時1人。帽子維持。正常位LoRA having missionary sex は c3-c5 に 0.4 まで。顔が消えたら外す。口内 CUMOUF なし | カナ、1本2行まで |
| 定期検診 | `checkup-100s` | 対面30秒。キス10＋ジュボ10。9本＝90秒。16:9 | 家の玄関。1本目は左手に医師が一人、チャイム＋こんにちは、右からレイがはい。かたくておっきいで軽くチンチンにキス→シツレイして、いっぱいさわっちゃいますね→キスと胸→ぬるぬるでモンダイない→カチカチおくまでカクニン→ジュボは射精せず抜いて正常位アナル中出し。「ふふ、モンダイありすぎ。アナルにだされすぎ、、、」 | カナ、1本2行まで |
| ケンシン | `clinic-75s` | 9本＝90秒。16:9。こんにちは→椅子座りジュボ（顔残し）→10秒ずっとジュボ→口を外すご褒美小便→まみれベロチュー→30度四つん這い接触→挿入→パコ→中出し | 医院にアヤが来る。医師が20cm。アヤ受け。レイなし。騎乗なし。女医は仰向けにしない。0-10 こんにちは→pipe chair に SEATED しゃがみジュボ（医師の顔はフレームに残る。PULLED BACK）。10-20 10秒ずっとジュボ。口を外すな。立つな。キスするな。20-30 口を外す。ありがとう！ゴホウビです→黄色い小便を顔へ。30-40 おしっこ、あったかーい！ありがとー！→濃厚ベロチュー。40-50 向き転換→LOOKBACK→両手床→両膝→自分で尻を開く→医師が後ろに膝→先端が outer ring of the anus entrance に接触（未挿入）。50-60 同じ姿勢のままその輪から根元まで挿入。60-70 パコパコ。70-80 アナル中出し（白）→抜いて垂れ。80-90 ゲンキになりましたか／はい、スッキリしました。c5-c9は黄色い小便まみれを落とさない。c5-c8は 30 degrees・SAME DIRECTION・NO KISS。rim / 縁 禁止 | カナ、1本2行まで |
| 終点 | `last-stop-40s` | 10×4 | 終点の車内。起こして跪いて咥える（竿舐め禁止）。ジュボで起こす | カナ、1本1行 |
| 終電 | `last-train-120s` | 10本＝100秒（全部10秒。15秒禁止）。車内ジュボ20秒→口内口移し→ホームでキス/ジュボ/アナル | 終点の延長。2人だけ。声かけはレイに向ける。口移しで起こしてホームへ。濃厚キス→ピロートーク→立ちジュボ→射精せず抜いて立ち後背アナル中出し→抱擁 | カナ、1本2行まで |
| ザーメン風呂 | `semen-bath-70s` | 5本＝50秒 | 家のおフロ。アヤが湯船、レイが20cmで白い粘液を溜める。口移しなし | カナ、1本2行まで |
| ニクカベ | `meat-wall-85s` | 7本＝70秒。歩行はうわぁ＋手繋ぎ＋軽いフレンチキス（咥えではない）、台詞10、ジュボ10・口内10無言 | 巨大生物の体内。コンクリートに肉を貼った部屋ではない。歩行の床は弾力（足は沈まない）。茶色い粘液は全身。白いおフロは廃油級のドロドロ（色は白）。レイは毎本フタナリ勃起20cm。ジュボは浸かり深さ判定なし。ジュボは射精せず抜いて立ち後背アナル中出し。家のザーメン風呂とは別 | カナ、1本2行まで |
| ニクカベ肥溜め | `meat-wall-cesspit-70s` | 7本＝70秒。冒頭は全身白ザーメン→肥溜め発見→肩まで浸かる→キス→ジュボするの→無言ジュボ→無言で四つん這いアナル中出し→アナルあつい | ニクカベ別バージョン。頭から足先まで濃い茶色の糞まみれ。ジュボは浸かり深さ判定なし。素のニクカベ（白いおフロ）とは別 | カナ、1本2行まで |
| カフェ | `cafe-100s` | 11本＝110秒。16:9 | 店員常時1人。着席いきかえる→座位ジュボ→口内白→ごっくん→接地→挿入→パコ→アナル中出し→立ち上がる→おなかいっぱいベロチュー。二人目の店員・クローン・トレイ二つ禁止 | カナ、1本2行まで |
| 車内販売 | `train-sales-80s` | 10×8 | おチャ＝ジュボ、ミルクコーヒー＝ジュボのあと後背位座位アナル中出し。レイの竿は使わない | カナ、2行まで |
| 赤信号 | `red-light-50s` | 10×5 | 信号待ち。運転レイ両手ハンドル、口アヤ。ジュボ→口内 | カナ、2行まで |
| ヨガ | `yoga-50s` | 10×5 | コツバン。四つん這いにアナル挿入→中出し（futa_anal） | カナ、2行まで |
| 背中流し | `back-wash-60s` | 10×6 | 洗う→マドカのマンコ舐め（cunnilingus_futa）→アガリユ＝ジュボ→対面座位アナル中出し | カナ、2行まで |
| カラオケ | `karaoke-50s` | 10×5 | 歌のあいだジュボ、最後の音で口内、点数 | カナ、2行まで |
| ランドリー | `laundromat-50s` | 10×5 | あと何分。洗濯機の縁で対面座位アナル挿入→中出し（futa_anal） | カナ、2行まで |
| 講義机 | `lecture-desk-50s` | 10×5 | 板書。教卓の下でジュボ→口内 | カナ、2行まで |
| キャンプ | `camp-50s` | 10×5 | 虫よけ。レイがアヤを舐める（cunnilingus_futa）→四つん這いアナル中出し | カナ、2行まで |
| 花火 | `fireworks-50s` | 10×5 | 上を見る。立ち後背アナル挿入→中出し（futa_anal）。顔は花火 | カナ、2行まで |
| 山小屋 | `cabin-40s` | 10×8 | 糞まみれで扉はアヤ一人。便座でシコシコ→ベロチュー→ジュボ→騎乗→後背→アナル中出しはアナルから溢れる | カナ、2行まで |
| ハイスイコウ | `manhole-30s` | 10×2 | 物語の追加。フタの会話→即ベロチュー＋胸揉み＋シコシコ→無言で立ち後背アナル中出し | カナ、1本目2行 |
| 屋上クーラー | `roof-ac-30s` | 10×2 | 物語の追加。会話→受け入れる立ち・アナル挿入寸前→無言で立ち後背アナル中出し | カナ、1本目2行 |
| ハマのテトラ | `tetrapod-30s` | 10×2 | 物語の追加。風の会話→即ベロチュー＋胸揉み＋シコシコ→無言で四つん這いアナル中出し | カナ、1本目2行 |
| 廃校ロッカー | `locker-30s` | 10×2 | 物語の追加。カギの会話→即ベロチュー＋胸揉み＋シコシコ→無言で立ち後背アナル中出し | カナ、1本目2行 |
| ドウロのど真ん中 | `crossing-30s` | 10×2 | 物語の追加。信号の会話→即ベロチュー＋胸揉み＋シコシコ→無言で立ち後背アナル中出し | カナ、1本目2行 |
| ガケの展望台 | `lookout-30s` | 10×2 | 物語の追加。霧の会話→即ベロチュー＋胸揉み＋シコシコ→無言で正常位アナル中出し | カナ、1本目2行 |
| コウジョウあと | `factory-30s` | 10×2 | 物語の追加。サビの会話→即ベロチュー＋胸揉み＋シコシコ→無言で後背位座位アナル中出し | カナ、1本目2行 |
| ガソリンスタンド跡 | `gas-station-30s` | 10×2 | 物語の追加。ミズの会話のあと口を開けて先端から手の幅（飲まない）→無言ジュボ口内。ジュボ側が同じ目線に立ち上がって濃厚キス口移し | カナ、1本目2行 |
| トンネル非常電話 | `tunnel-phone-30s` | 10×2 | 物語の追加。電話の会話→受話器持ったまま跪いて口を開けて先端から手の幅→口だけ。ジュボ側が同じ目線に立ち上がって濃厚キス口移し | カナ、1本目2行 |
| 川原のゴミ | `riverbank-30s` | 10×2 | 物語の追加。フクロの会話→即ベロチュー＋胸揉み＋シコシコ→無言で騎乗位アナル中出し | カナ、1本目2行 |

...
- `validate_story_follow()` は 10秒／行為中は無言／口元の寄り／接合点（futa_sex と doggy）に加えて **hmmotion は AIO セックス本だけ・先頭**、ユニーク「」は1本に `spoken_max`（既定1、上限2）まで、**「」内に漢字なし**、Clear futanari は玉なし＋マンコあり を検査する

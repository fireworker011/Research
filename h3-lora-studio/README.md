...
体位は場面ごとに一番再現しやすいものを当てる（正常位・後背位・立ち後背位・騎乗位・後輩騎乗位・座位・後背位座位）。結合部が見える後背位／立ち後背位を多めにするが、座り物や仰向けの場面に捩じ込まない。前の本の終わりが必ず**受け入れる姿勢＋穴から手の幅・未挿入**で終わり、次の本が `INSERTION ON CAMERA` から始まる。挿入開始は1シーン早め、空いた枠はパコパコ（亀頭が rim まで抜けてから BASE まで打ち込む。完全には抜かない）。挿入中は双方の恍惚・声・口元のよだれ。ジュボの秒数とセリフは削らない。

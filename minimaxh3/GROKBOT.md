...
### みなと埠頭クリア（全年齢・横スクロール 1撃・ミッション完了）

病棟の話は使わない。カメラと一撃の書き方だけ借りる。ノートの既定 EPISODE は変えない。
歩きは A100 40GB。格闘 LoRA は A100 80GB（VRAM ≥70GB）のときだけ。Colab の High-RAM では積まない。

```
skill h3-episode-grokbot を読む。
質問しない。inbox は触るな。I2V/T2V/R2V ランナーは動かすな。Imagine するな。
python minimaxh3/grokbot/run_episode.py --episode minato-ramp-clear --branch cursor/h3-minato-ramp-clear-33d9
投稿するな。Wan/Max/完全版ノート/loca.lt/LoRAスタジオは使うな。終わったら colab stop。
結果は slug と Drive episodes/minato-ramp-clear/final/latest.mp4 のパスだけ。失敗なら status.json のエラー一行。
```

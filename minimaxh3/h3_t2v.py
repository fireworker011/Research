...
with first_frame / last_frame unwired. 9:16 Shorts canvas is 576x1024
(multiples of 32). Affiliate URLs never go in prompts.
"""
...
def t2v_retry_plans(*, width: int, height: int) -> list[dict[str, Any]]:
    ...
    area = w * h
    for cw, ch in CANVAS_9_16_LADDER:
        if cw * ch < area:
...
def resolve_t2v_prompt(prompt: str | None) -> str:
    text = (prompt or "").strip()
    return text or DEFAULT_T2V_PROMPT
...
def assert_t2v_graph(g: dict[str, Any]) -> list[str]:
    ...
    if not is_9_16(w, h) and (w, h) != CANVAS_9_16_HIGH:
        errs.append("canvas should stay 9:16 (or official 768x1344)")

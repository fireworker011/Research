...
借りるのは三人称カメラ・カバー・覗き・失敗カードの文法だけ。定時退社シミュレーターの固有名は使わない。

カバー 3.0 秒 → 覗き 5.0 秒 → 同僚（字幕のみ・自席完了）6.0 秒 → かばんメニュー 2.2 秒 → 着席 4.8 秒 → ミッション失敗「隣の席に座った」→ 免責。
しゃがみは残す。首と胴が同じ方向の横向き。

次版のたたき（コマンド四択＋警備員への Combat LoRA、json はまだ変えない）は `SCRIPT.md`。

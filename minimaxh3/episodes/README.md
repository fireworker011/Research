...
`tone: mundane`。爆発・キック・エレベーター・喫煙の階段は書かない。しゃがみカバーは残し、首と胴が同じ方向の横向きにする。Colab は `EPISODE = "kasumi-late-desk"`（マージ前は `BRANCH` をこの PR ブランチ）。inbox には置かない。準備は `episodes/kasumi-late-desk/PREP.md`。

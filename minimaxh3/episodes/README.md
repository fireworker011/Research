...
- 土台 UNet は `render.lane` + `render.checkpoint` で固定する。省略時はどちらも `stock`（`minimax_h3_fl2va_pruned_int8_convrot.safetensors`）。`eros-max` は `lane: erotic` のときだけ。あさは Drive の `models/diffusion_models/10Eros_Max_h3_TURBO-hybrid_beta5_int8.safetensors`（約21GB）を使う。無ければ HuggingFace `TenStrip/10Eros-Max` から取る。`models/erotic/` へリンクし、inbox / 非エロ予告の `*fl2va*` 先頭取りは 10Eros を飛ばす。霞東本体・番台は stock 固定。`*-adult` は erotic + eros-max 必須。LightX2V turbo LoRA はこの UNet に積まない（turbo は焼き込み済み）。DT-sQKV は使わない

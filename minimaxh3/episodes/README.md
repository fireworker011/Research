...
オフ約 55 秒・オン約 48 秒。Colab 4 番で話が分かれる。オフ=じゅぼ口内口移し（なな座位フェード）・後ろから接合中出し（青木完全フェード）・挿入開始から仰向けのまま腰（戦いなし）。オン=横スク戦い＋じゅぼ＋仰向け接合。失敗は「正常位で動けない」。バトルは参考約12秒の半分（5秒）。オフのななじゅぼは 10 秒フル。青木中出しは 7.5 秒。竿は 24cm。セックスは体位名ではなく誰がどの方向へ（病棟と同じ）。GPU のつなぎは Colab で選ぶ（迷ったらカット＝T2V。前の最終フレームから続ける／用意した最終フレームへ着くも可）。既定カメラは `side2d`。`action3d` に切替可。プリセット既定は `balance`。じゅぼは物語集 blowjob LoRA（`bl0w_j0b`）。オンのとき Combat は 06 と 10（`prfight2, prfin1`）。UNet は erotic + eros-max。本体・番台は stock。Colab は `EPISODE = "kasumi-late-desk-adult"`（マージ前は `BRANCH=cursor/h3-kasumi-adult-0402`）。霞東 raw は reuse するな。参考 mp4 はモーションにしない。

## 病棟出口（`hospital-exit-adult`）

同じ枝の別スラッグ。感染者だらけの古いボロボロの架空病院から出る（パンデミックで傷んだ。実在病名は出さない）。敵は全裸の成人女性かふたなり。血は出ない。**Colab の話で病棟出口を選ぶ。5 番が全体の既定。シーンごとで誘う（誘い方含む）・受け入れる・回避を上書き。戦い構成のときはシーンごとは無視。完了／失敗は最後に残った人。** 受け入れる＝接合のまま出口・完了。誘う＝途中は次へ、最後だけ淫欲に呑まれた・失敗。回避＝一人で出口・完了。戦って勝つ＝倒して出口・完了。戦って負ける＝敗北H・失敗。6番は誘うポーズ。7番は道中トイレ（どれでも次へ）。登場チェックでみき／れい／かな／しのを飛ばせる（ゼロ人はエラー）。4 番 Combat は戦い構成＋ハイメモリだけ。れい 24cm・かな 20cm（3番目）・しの 30cm（4番目。灰白の汚れ・怪物の目・妖艶・長い舌）・みきふたなり 22cm。主人公以外は全員ふたなり。セックスは誰がどの方向へ動くかを書く（霞東も同じ。体位名だけにしない）。GPU のつなぎは Colab で選ぶ（迷ったらカット）。UNet erotic + eros-max。スチールは病棟廊下だけ（霞東ヌードを流用しない）。スラッグは `hospital-exit-adult`。続きは `episodes/hospital-exit-adult/`。

...
25秒・ミッション失敗。参照は「カバーに隠れてすり抜け、HUD は後載せ、最後は失敗カード」の文法だけ。学校・制服・段ボール箱・原クリップは使わない（成人の厨房、クレート、タイムカード）。
クレート 3.0秒 → 路地歩き 5.0秒 → 厨房（字幕のみ・伝票完了）6.0秒 → 道具メニュー 2.2秒 → 打刻 4.8秒 → ミッション失敗「タイムカードを裏向きに挿した」→ 免責。
Colab は `EPISODE = "kogane-timecard"`。inbox には置かない。

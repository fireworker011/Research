...
- `04-gin-ride`: その続きの着座。入れ方は `SQUATS from above`、`from directly above`、`from above`。竿はあやの股間から真上。あやの頭は右、足先は左。ギンの頭は左。誘う側の騎乗にはこの着座の文を足さない
...
横からの距離固定。あやの顔と相手の顔が残る。全身、両足。誘う側の camera と action から `from directly above` と `SQUATS from above` は落とす。着座は、腰を亀頭の高さまで上げ、両足を右側の竿の位置まで滑らせ、そのまま真下へ下ろして中へ。灰色の `04-gin-ride` の action だけ、`SQUATS from above` の着座を残す。足を滑らせる文は足さない。
...
小便はマンコから黄色い液体。便座の外へ流す。あやは笑顔。便器の中へ流す文は書かない。
...
口の最終コマは跪き、唇が根元。次の着座は跪きからかなを仰向けにして腰に座る。腰を亀頭の高さまで上げ、両足を右側の竿の位置まで滑らせ、そのまま真下へ下ろす。3段下ろし。口カットの途中で着座へ変形しない。
...
指示がなければ寄らない。`zoom` は書かない。ワイド全身のまま、頭と股と両足が同じ大きさでフレームに残る。冒頭のミキは 22cm もフレームに残る。横スク lock の `tracks only left and right` は距離固定に置き換える。`01-cover` のキスで寄らない。

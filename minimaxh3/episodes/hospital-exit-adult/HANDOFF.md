...
- `04-tsuno-meet`: 停止から背後の熱烈な抱擁と胸揉み。抱きつかれた瞬間、あやは嬉しそうな顔で受け入れる。密着したまま少しずつ壁へ移る動きは既存。LUNGES と SNAPS は使わない。フルネルソンの meet も同じ抱擁から入り、文は `wraps Aya from behind`
...
横からの距離固定。あやの顔と相手の顔が残る。全身、両足。誘う側の camera と action から `from directly above` と `SQUATS from above` は落とす。着座は横顔のまま腰が下る。灰色の `04-gin-ride` の action だけ、その着座の文を残す。

...
スタートは全身。ギン以外の口、かなの顔射、キスは、二人の顔が画面の中に全部残る。竿役の顔も全部残る。カメラは顔が切れる距離まで寄らない。`zoom` と `Do not push the camera in` は書かない。正の文だけ。挿入に移る前に、カメラはまた全身まで引く。頭も足も全部。ギンの `04-gin-jupo` はこの顔フレームを足さない。

...
- `04-gin-jupo`: 既存のまま。あやは座ったまま仰向けになる。頭は右、足先は左のまま。両足はリノリウムに付き、画面に残る。24cm はあやの股間から垂直に真上。ギンの頭は左。腰の上に屈み、舌は口の中、唇でその垂直の竿を根元まで咬える。片手は根元を持つ。あやの顔は目を細めた恍惚の笑顔。最後のコマは、あやが仰向け、竿が真上、ギンが跪いて胸と竿を持ち、マンコが亀頭の真上。次の `04-gin-ride` の入り
- そこから既存の分岐（騎乗 / 正常位 / 後背）。成長とジュボは別カット
- `04-gin-ride`: あやは RIGHT で両手を後ろについて座り、膝は開く。24cm は股間から上。Gin STANDS UP のあと STEPS RIGHT。両足裏は床の RIGHT third、あやの膝の両脇に一本ずつ。片手で 24cm を持ち、亀頭を無毛のマンコに当て、LOWERS her hips straight down で根元まで。ギンの尻があやの腰に当たる。そこで止める。両足はリノリウム。最後は RIGHT third でギンがあやに座り、腰は密着、24cm は根元まで、両手は竿から離れる
- `04-gin-peak`: 同じ場所で既に結合。ギンは床の RIGHT third であやに座っている。24cm は根元まで入ったまま。両手はあやの胸。短い上下。亀頭は中に残る。腰は密着に戻る。中出しの文は書かない
...
ミキは歩いている。あやが後ろから抱きついた瞬間に、ミキの足が開いたリノリウムの上で止まる。壁まで歩いてから止まらない。その停止と同じ瞬間、右側に T 字路の壁が見えてくる。足と壁のあいだにリノリウムが残る。抱きつかれた瞬間、ミキは空の眼窩のまま不気味な笑顔。`zombie` `corpse` `blood` は書かない。胸・腹・腰・腿は隙間なく密着。片手で胸を揉み、もう片方の手で勃起した 24cm を上下する。振り向くとき両手は竿から離れる。そのあとのべろちゅーが残りの尺。このカットだけ `cast_lock` で 24cm。他のミキは lock の 22cm のまま。歩きは全身。口が付いたあとは、膝の上から顔まで。あやの顔とミキの顔は両方、画面の中。
...
- 次: 竿役がしゃがみ、舌。トリガー `performing cunnilingus`。cunny 0.8
- 次: 両腕で両腿を抱え、両足は床から離れる。竿はマンコの根元まで。体位名は書かない
- 最後はあや一人が右へ歩く
...
着座の sideride は 0.5。着座の mystic はそのまま。絶頂の sideride は 0.8。絶頂に mystic は重ねない。penis 0.45、synth 0.4、thrust 0.55 は絶頂に残す。ファイルは `cowgirl-side-2-mh3-e50-az420.safetensors` のまま。あやのロックは `female body`。penis と futanari の否定はあやに書かない。

## 対面座位

誘い方の `sit` は `invite_pose_sit`。みき、れい、かな、しのの4カットにある。LoRA は kiss 0.5。

- みきの `03-kiss-zai1`: あやは LEFT で跪き、唇が勃起した 22cm にある。ミキは RIGHT で立っている。口が竿を離す。ミキはその RIGHT のリノリウムに座る。胴は直立、膝は曲がり、両足は床。22cm は股間から上。あやは跪きから立ち、ミキへ寄って向かい合って座る。脚はミキの腰、あやの腕は肩、ミキの腕はあやの腰。胸が密着。あやが腰を下ろし、22cm がマンコの根元まで入って腰が密着したら静止。このカットでは腰を上下しない
- れい・かな・しのの `zai1`: 竿役が床に尻をつく。先は真上。あやが腰を垂直に下ろして根元まで入れ、そこで止める。太ももと尻が付き、竿は見えなくなる。あやの両足は相手の足の手前から背中へ回り、足首が背中の後ろでクロス。口は付いたまま。このカットでは腰を上下しない
...
`04-gin-ride` はあやが RIGHT。Gin STANDS UP のあと STEPS RIGHT。足裏は床の RIGHT third、膝の両脇に一本ずつ。握って先を当て、LOWERS her hips straight down で根元まで。尻があやの腰に当たったら止める。両手は最後に竿から離れる。`04-gin-peak` は RIGHT third で入ったまま、両手を胸に置いて短い上下。亀頭は中。中出しの文は書かない。しゃがむ語、両足を同じ側、上下と停止の同時指示は書かない。04 の歩きは出口ではない。オプションを全部入れてもギンの次は `04-tsuno-meet-spot`。角の次の拍は `04-dog-spot`。犬の LoRA はページ URL しか無く、保存された HTML を重みとして読むと Comfy がそこで落ち、角の動画のあと生成が止まる。HTML は消して、その拍は LoRA なしで続ける。ミッション完了は最後の拍だけ。誘うでは最後も完了にしない。

## 騎乗のカメラ

横からの距離固定。全身、両足。同じ大きさ。着座は短い順。竿役が完全に仰向け、頭は右、足先は左、頭と肩が床。竿は股間のあいだに根があり、亀頭が上端、真上のまま、その股間から動かない。受け入れ側の頭は左。同じ向きのまま一度立つ。足は竿役の脚の外側。視聴者から見て手前の足は竿役の手前の脚より前、奥の足は竿役の奥の脚より後ろ。足裏は尻の真横。腰を落とす位置はさらに竿役の顔側。両手は胸。その位置のまま下ろすと亀頭がマンコに入る。次の絶頂もその足の位置のまま、両手は胸に置ける。マンコは亀頭の真上。亀頭はマンコの真下。股間から亀頭、マンコまで一本の縦線。立ち、足、スクワット、挿入まで同じ向き。そこで挿入完了。そのカットは止める。上下は次の絶頂。絶頂は画面の縦に、まっすぐ上、まっすぐ下。頭は右と左のまま、竿は股間から真上。着座も絶頂も、画面の二人はあやと相手だけ。挿入の瞬間、立っていた竿役が床の顔になる。跪いていたあやが上の顔になる。挿入後もその二つの顔。絶頂のトリガーも、すでに仰向けの大人の竿が中で終わる文にする。`nothing new enters` は騎乗に書かない。入ってくる人として描かれる。

...
- `04-gin-lick`: あやがギンのいる左へ体ごと恐る恐る振り向き、驚きの顔で尻餅。そこがゴール。最終はギンが左でしゃがみ舌が出、あやが右で座り両膝開き 24cm 真上。舌と 24cm 成長は尻餅の後に同じ。ギンは竿なし。`futanari` の語は書かない。右端は暗い通路が続く。壁という語は書かない
...
横からの距離固定。あやの顔と相手の顔が残る。全身、両足。プロンプトの camera と action から `from directly above` と `SQUATS from above` は落とす。着座は横顔のまま腰が下る。
...
触手は3本。口へ上がる1本とマンコへ入る1本はそのまま。アナルの1本は便座の下から、視聴者からは便器の中から出て肛門へ。太腿を押さえる余分な触手は書かない。

...
- 台本の生成: `colab/_write_rei_escape_episode.py` を直し、`python3 colab/_write_rei_escape_episode.py` で `episode.json` を出す。json を手で直さない
...
レイ: 21歳、茶髪ロングストレート、スレンダー、くびれ、Cカップ、ふたなり、絶対全裸、素足。汗の粒と茶色い汚れが全身と竿に読める。普段の顔は口を閉じ、舌は口の中。舌を出すのは、そのカットの動作がじゅぼの快感かイキ顔と書いたときだけ。主人公の名前はレイだけ。16 までは 24cm はレイの竿。17 で切れ目だけになり、24cm はサキュバスへ移る。22 以降は No penis / The grown shaft is gone。汗と汚れは残す。

敵1: 古城の石柱と消えた炊醬から出る窄み円口。質量はレイの2倍。円口はひょっとこ状に窄めた唇環。大口の漏斗にしない。人間の顔・手・唇を出さない。円口の行為は mystic のみ。口 LoRA は載せない。

蛾スロット: 蜘蚪女。上半身はレイと同じ背丈の裸体の女性。下半身は節腹、多脚、尾。尾の先端の円口。森は出さない。場所は古城。

サキュバス: 黒髪、赤い目、黒い角、顔の紋、チョーカー、破れた翼、長い尾、爪。全裸、無毛。翼・角・チョーカー・紋・爪は残す。17 以降は penis growth の 24cm がサキュバス側。20–21 の挿入側はサキュバス。中出しはレイ。

場所は廃墟のゴシック古城。広間（尖頭窓・右階段）は立ちと遭遇。回廊（ヴォールト・天井穴）は走り。`world.bare_set` は false。看板・テレビは出さない。壁の垂れ布は石に付いたまま。レイには載せない。床の赤みは書かない。

トイレは食虫植物の着座形。緑の外皮、内側は湿った赤、蜜色の液。陶器は出さない。脱糞は書かない。トイレのあと肌に糞を塗らない（`DIRTY-STATE` は今の生成では付かない）。
...
- 新しい相手の入りは台本が `connect: t2v` で固定。チェーンを選んでも T2V のまま。対象は `03-slime`、`04-enemy1`、`07-hearth`、`08-toilet`、`11-leech`、`12-moth`、`15-tentacles`、`16-succ`
- 同じ相手の次の行為は I2V。例: `05-enemy1-maw`、`17-attack`、`20-fours-in`、`21-orgasm`
...
- 合間おなのドロップダウンは無い。`03-slime` `07-hearth` `11-leech` `15-tentacles` は常時 T2V。extra は mystic
- トイレ: 着座おな射精（`09-ta`）。小便は着座。触手は植物便座の円口が 24cm を根元まで吞む（`09-tc`、mystic）
- 敵1: 受け入れる＝立ち円口（`05-enemy1-maw`）。誘う＝その場で仰向け、笑顔で膝を開く（`05-enemy1-invite`）。回避＝その行為を飛ばしてフェードから走り。回避の走りに射精の文は書かない
- 蛾スロット: 尾端の第二口（`13-tail`）。両方立ち止まる。上の口を選ぶと唇と舌（`13-mouth`、blowjob LoRA）
- 変化: ドロップダウンは無い。`16-succ` は触れる直前。竿はまだレイ。`17-attack` は overlay なし。触れて futanari transformation。レイの竿が消え切れ目だけ。サキュバスに penis growth 24cm。extra は futatf 1.0
- キス: 既定オン（`18-kiss`）。正面抱擁。本文は They kiss passionately。extra は kiss 1.0。オフにするとこのカットは出ない
- 口: 既定オン（`19-oral`）。サキュバスは立ち、レイは股を開いてしゃがむ。唇がサキュバスの 24cm を往復。片手は自分の切れ目、片手は腿から腰。extra は blowjob 0.8。体位のカットにキスや口を戻さない
- 挿入: 挿入側はサキュバス。中出しはレイ。四つん這い後ろからが既定（`20-fours-in` のあと `20-fours-out`）。壁に手はレイの掌を壁、足は固定、サキュバスの竿が中。沈めて上げ下ろしはレイが仰向け。仰向け膝開きもサキュバスが入れる。体位名は本文に書かない
- LoRA: futatf は `MiniMax-H3_Futa_Transformations_LoRA_V5.1.safetensors`（モデル 3212000、強度 1.0）。kiss は `MM-H3 - Passionate Kiss.safetensors`（モデル 3289478、強度 1.0）。cxy のキスファイルは使わない。Drive は `models/loras`。鍵は `CIVITAI_API_TOKEN`

...
セックスは体位名ではなく誰がどの方向へ（病棟と同じ）。挿入は竿役の腰と受け入れ側の腰を両方書く。
...
同枝に別スラッグ `hospital-exit-adult`（病棟脱出。Colab 5 番で完了／失敗。みき竿なし・れい 24cm・かな 20cm 3番目・しの 24cm 4番目。登場スキップ・誘うポーズ・道中トイレあり）。
